...
Cypress.Commands.add('getEholdingsRootProxyViaAPI', () => {
  cy.okapiRequest({
    method: 'GET',
    path: 'eholdings/root-proxy',
    isDefaultSearchParamsRequired: false,
  }).then((rootResponse) => {
    return rootResponse.body.data;
  });
});
...
Cypress.Commands.add('getEholdingsProxiesViaAPI', () => {
  cy.okapiRequest({
    method: 'GET',
    path: 'eholdings/proxy-types?limit=20',
    isDefaultSearchParamsRequired: false,
  });
});

Cypress.Commands.add('getEholdingsProxyNamesViaAPI', () => {
  cy.getEholdingsRootProxyViaAPI().then((rootProxy) => {
    cy.getEholdingsProxiesViaAPI().then(({ body }) => {
      return body.data.map((proxy) => {
        if (proxy.attributes.id === rootProxy.attributes.proxyTypeId) return `Inherited - ${proxy.attributes.name}`;
