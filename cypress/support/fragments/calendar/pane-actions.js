...
export default {
  newButtonExists() {
    cy.do(Button('New').exists());
  },
  deleteButtonExists() {
    cy.do(Button('Delete').exists());
  },
  editButtonExists() {
    cy.do(Button('Edit').exists());
  },
  duplicateButtonExists() {
    cy.do(Button('Duplicate').exists());
  },
  purgeOldCalendarsButtonExists() {
    cy.do(Button('Purge old calendars').exists());
  },
  checkPaneExists(paneHeading) {
    cy.do(Pane(paneHeading).exists());
  },
  calendarTable: {
    clickCalendarNameHeader() {
      cy.do(MultiColumnListHeader('Calendar name').click());
    },
    clickStartDateHeader() {
      cy.do(MultiColumnListHeader('Start date').click());
    },
    clickEndDateHeader() {
      cy.do(MultiColumnListHeader('End date').click());
    },
    clickAssignmentsHeader() {
      cy.do(MultiColumnListHeader('Assignments').click());
    },
    clickServicePointHeader() {
      cy.do(MultiColumnListHeader('Service point').click());
    },
    clickCurrentStatusHeader() {
      cy.do(MultiColumnListHeader('Current status').click());
    },
  },
  openCalendarWithServicePoint(servicePoint) {
    cy.do([
      ...
      Pane('Current calendar assignments')
        .find(MultiColumnListCell(servicePoint, { column: 'Service point' }))
        .click(),
    ]);
  },
  allCalendarsPane: {
    clickPurgeOldCalendarsAction() {
      cy.do(Pane('All calendars').clickAction('Purge old calendars'));
    },
    clickNewButton() {
      cy.do([
    ...
    checkActionMenuAbsent() {
      ...
      cy.do([Pane('All calendars').find(PaneActionButton).absent()]);
    },
    checkActionMenuPresent() {
      const PaneActionButton = Button({ className: including('actionMenuToggle') });

      cy.do([Pane('All calendars').find(PaneActionButton).exists()]);
    },
    openActionMenu() {
      const PaneActionButton = Button({ className: including('actionMenuToggle') });

      cy.do([Pane('All calendars').find(PaneActionButton).click()]);
    },
    selectCalendar(calendarName) {
      cy.do([
        Pane('All calendars')
          .find(MultiColumnListCell(calendarName, { column: 'Calendar name' }))
          .click(),
      ]);
    },
    ...
    checkCalendarAbsent(calendarName) {
      cy.do(Pane('All calendars').find(MultiColumnListCell(calendarName)).absent());
    },
  ...
  individualCalendarPane: {
    checkIndividualCalendarPaneExists(calendarName) {
      cy.do([Pane(calendarName).exists()]);
    },
    checkActionMenuAbsent(calendarName) {
      const PaneActionButton = Button({ className: including('actionMenuToggle') });

      cy.do([Pane(calendarName).find(PaneActionButton).absent()]);
    },
    checkActionMenuPresent(calendarName) {
      const PaneActionButton = Button({ className: including('actionMenuToggle') });

      cy.do([Pane(calendarName).find(PaneActionButton).exists()]);
    },
    openActionMenu(calendarName) {
      const PaneActionButton = Button({ className: including('actionMenuToggle') });

      cy.do([Pane(calendarName).find(PaneActionButton).click()]);
    },
    close(calendarName) {
      cy.do(
        Pane(calendarName)
          .find(Button({ ariaLabel: 'Close ' + calendarName }))
          .click(),
      );
    },
    checkIfOpen247(calendarName) {
      ...
      daysOfTheWeek.forEach((day) => {
        const row = hoursOfOperationAccordion.find(
          MultiColumnListRow({ content: including(day), isContainer: true }),
        );
        ...
        jobs.push(
          row
            .find(
              MultiColumnListCell({
                column: 'Open',
                innerHTML: '<p title="This calendar is open 24/7 and does not close">–</p>',
              }),
            )
            .exists(),
        );
      ...
      cy.do([
        hoursOfOperationAccordion
          .find(Paragraph('This calendar is open 24/7 and does not close'))
          .exists(),
        ...jobs,
      ]);
    },
    checkOpeningExceptions({
      calendarName,
      ...
      const row = Pane(calendarName)
        ...
        .find(
          MultiColumnListRow({
            content: including(addExceptionsOpeningData.name),
            isContainer: true,
          }),
        );
    ...
    selectDuplicateAction({ calendarName }) {
      cy.do(Pane(calendarName).clickAction('Duplicate'));
    },
    selectDeleteAction({ calendarName }) {
      cy.do(Pane(calendarName).clickAction('Delete'));
    },
    ...
    checkDeleteHoursOfOperation({ calendarName, openingHoursData }) {
      cy.do([
        Pane(calendarName)
          ...
          .find(
            MultiColumnListRow({
              content: including(openingHoursData.startDay),
              isContainer: true,
            }),
          )
        ...
        Pane(calendarName)
          .find(Accordion('Hours of operation'))
          .find(
            MultiColumnListRow({ content: including(openingHoursData.endDay), isContainer: true }),
          )
          .find(MultiColumnListCell({ column: 'Open', content: 'Closed' }))
          .exists(),
      ]);
    ...
    checkEditExistingCalendars(editExistingCalendarsData) {
      ...
      const hoursOfOperationAccordion = Pane(editExistingCalendarsData.name).find(
        Accordion('Hours of operation'),
      );
    ...
    checkEditHoursOfOperation({ calendar, editHoursOfOperationExpectedUIValues }) {
      const hoursOfOperationStartRow = Pane(calendar.name)
        .find(Accordion('Hours of operation'))
        .find(
          MultiColumnListRow({
            content: including(calendar.normalHours[0].startDay),
            isContainer: true,
          }),
        );
      const jobs = [];
      ...
      if (calendar.normalHours[0].startDay === calendar.normalHours[0].endDay) {
        jobs.push(
          hoursOfOperationStartRow
            .find(
              MultiColumnListCell({
                column: 'Open',
                content: including(editHoursOfOperationExpectedUIValues.startTime),
              }),
            )
            .exists(),
          hoursOfOperationStartRow
            .find(
              MultiColumnListCell({
                column: 'Close',
                content: including(editHoursOfOperationExpectedUIValues.endTime),
              }),
            )
            .exists(),
        );
      } else {
        const hoursOfOperationEndRow = Pane(calendar.name)
          .find(Accordion('Hours of operation'))
          .find(
            MultiColumnListRow({
              content: including(calendar.normalHours[0].endDay),
              isContainer: true,
            }),
          );
  ...
  currentCalendarAssignmentsPane: {
    ...
    checkNewButtonExists() {
      cy.do([PaneHeader('Current calendar assignments').find(Button('New')).exists()]);
    },
    checkNewButtonAbsent() {
      cy.do([PaneHeader('Current calendar assignments').find(Button('New')).absent()]);
    },
  ...
  monthlyCalendarView: {
    ...
    checkPrevAndNextButtons({ servicePointName }) {
      ...
      cy.do([
        Pane(servicePointName)
          .find(Headline(including(months[currentDate.getMonth()])))
          .exists(),
      ]);
      ...
      cy.do([
        Pane(servicePointName)
          .find(Button({ ariaLabel: 'arrow-left' }))
          .click(),
        Pane(servicePointName)
          .find(Headline(including(prevMonth)))
          .exists(),
      ]);

      cy.do([
        Pane(servicePointName)
          .find(Button({ ariaLabel: 'arrow-right' }))
          .click(),
        Pane(servicePointName)
          .find(Headline(including(months[currentDate.getMonth()])))
          .exists(),
      ...
      cy.do([
        Pane(servicePointName)
          ...
          .click(),
        Pane(servicePointName)
          .find(Headline(including(nextMonth)))
          .exists(),
        Pane(servicePointName)
          .find(Button({ ariaLabel: 'arrow-left' }))
          .click(),
    ...
    checkCalendarCells({ calendar, monthlyCalendarViewExpectedUIValues }) {
      ...
      cy.do(CalendarCell({ inCurrentMonth: false, content: not('Closed') }).absent());
      ...
      while (currDay <= lastDay) {
        ...
        if (currDate in monthlyCalendarViewExpectedUIValues.exceptions) {
          ...
          cy.do(
            CalendarCell({
              dayLabel: currDay.toString(),
              content: monthlyCalendarViewExpectedUIValues.days[days[dayOfWeekIndex]],
            }).exists(),
          );
        }
