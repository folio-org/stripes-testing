...
const browseSearchAndFilterInput = Select({
  id: "input-record-search-qindex",
});
...
export default {
  ...
  getSearchResult: (row = 0, col = 0) =>
    MultiColumnListCell({ row: row, columnIndex: col }),
  waitLoading: () =>
    cy.expect([Form().find(inventorySearchAndFilter).exists()]),
  ...
  byKeywords(kw = "*") {
    return cy.do([keywordInput.fillIn(kw), searchButton.click()]);
  },
  ...
  verifyCallNumberBrowseEmptyPane() {
    ...
    cy.expect(
      HTML(
        including("Choose a filter or enter a search query to show results.")
      ).exists()
    );
  },
