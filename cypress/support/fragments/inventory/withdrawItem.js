...
const withdrawItemButton = Button('Mark as withdrawn');
...
export default {
  ...
  getItemsToWithdraw(items) {
    return items.filter(({ status: { name } }) => this.itemsToWithdraw.includes(name));
  },

  getItemsNotToWithdraw(items) {
    return items.filter(({ status: { name } }) => this.itemsNotToWithdraw.includes(name));
  },
  ...
  createItemsForGivenStatusesApi() {
    ...
    return cy.wrap(Promise.resolve(true)).then(() => {
      ...
      const items = [
        ...this.itemsToWithdraw,
        ...this.itemsNotToWithdraw
      ].map(status => ({
        itemId: uuid(),
        barcode: uuid(),
        status: { name: status },
  ...
  clickMarkAsWithdrawn() {
    cy.do([actionsButton.click(), withdrawItemButton.click()]);
  },

  checkIsMarkAsWithdrawnExist(isExist) {
    cy.do(actionsButton.click());
    if (isExist) {
      cy.expect(withdrawItemButton.exists());
    } else {
      cy.expect(withdrawItemButton.absent());
    }
    cy.do(actionsButton.click());
  },
