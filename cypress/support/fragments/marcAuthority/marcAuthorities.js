...
export default {
  ...
  chooseTypeOfHeading: (headingTypes) => {
    ...
    cy.then(() => headingTypeAccordion.open()).then((isOpen) => {
      if (!isOpen) {
        cy.intercept(/search\/authorities\/facets\?facet=.*headingType.*/).as(
          'getFacetsHeadingType',
        );
  ...
  checkResultsListRecordsCountGreaterThan(totalRecord) {
    ...
    cy.wait('@getItems', { timeout: 10000 }).then((item) => {
      ...
      cy.expect(
        Pane({
          subtitle: matching(
            new RegExp(`${totalRecords.toLocaleString('en-US')} (record|result)s{0,1} found`),
          ),
        }).exists(),
      );
  ...
  verifyActionsMenu(sortOption = sortOptions[0]) {
    cy.expect([
