export default {
  ...
  getJobProfileQueryString({ profileId }) {
    return `limit=100&profileIdAny=${profileId}&sortBy=completed_date%2Cdesc&statusAny=COMMITTED&statusAny=ERROR`;
  },

  getUserQueryString({ userId }) {
    return `limit=100&sortBy=completed_date%2Cdesc&statusAny=COMMITTED&statusAny=ERROR&userId=${userId}`;
  },
  ...
  getInventorySingleRecordImportsQueryString({ filter }) {
    if (filter === 'Yes') {
      return 'limit=100&' +
        ...
        'statusAny=COMMITTED&statusAny=ERROR';
    }
    return 'limit=100&' +
