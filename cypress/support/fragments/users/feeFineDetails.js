import { Pane, including, HTML, Button, Modal, TextField } from '../../../../interactors';
...
export default {
  ...
  openErrorModal: () => cy.do(rootPane.find(Button({ id:'errorAccountActionsHistory' })).click()),
  ...
  confirmFeeFineCancellation: (comment) => {
    cy.do([
      Modal({ id: 'error-modal' }).find(TextField({ name: 'comment' })).fillIn(comment),
