...
export default {
  ...
  verifyResultCells(verifyDate = false) {
    ...
    function getResultRowByRowNumber(rowNumber) {
      return {
        '#list-column-timereturned': MultiColumnListCell({ row: rowNumber, columnIndex: 0, content: matching(dateRegEx) }),
        '#list-column-title': MultiColumnListCell({ row: rowNumber, columnIndex: 1, content: matching(/((\d|\w)\s*)/) }),
        '#list-column-barcode': MultiColumnListCell({ row: rowNumber, columnIndex: 2, content: matching(/\d|-/) }),
        '#list-column-effectivecallnumber': MultiColumnListCell({ row: rowNumber, columnIndex: 3, content: matching(/\w|/) }),
        '#list-column-location': MultiColumnListCell({ row: rowNumber, columnIndex: 4, content: matching(/\w/) }),
        '#list-column-inhouseuse': MultiColumnListCell({ row: rowNumber, columnIndex: 5, content: matching(/\w|/) }),
        '#list-column-status': MultiColumnListCell({ row: rowNumber, columnIndex: 6, content: matching(/\w\s*-*\d*/) }),
      };
    }
    ...
    return cy.get('#list-items-checked-in').then(element => {
      ...
      const resultCount = element.attr('aria-rowcount') > 29 ? 29 : element.attr('aria-rowcount') - 1;
