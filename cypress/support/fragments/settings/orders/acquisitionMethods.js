...
export default {
  ...
  editAcquisitionMethod: (AMName) => {
    cy.do(MultiColumnListCell({ content: AMName }).perform(
      element => {
    ...
    cy.do([
      TextField().fillIn(`${AMName}-edited`),
      acquisitionMethodPane.find(saveButton).click()
    ]);
  },

  deleteAcquisitionMethod: (AMName) => {
    cy.do(MultiColumnListCell({ content: `${AMName}-edited` }).perform(
      element => {
        const rowNumber = element.parentElement.parentElement.getAttribute('data-row-index');
    ...
    InteractorsTools.checkCalloutMessage(`The acquisition method ${AMName}-edited was successfully deleted`);
  },
