...
const deleteOwner = (rowNumber = 2) => {
  cy.do(
    tableWithOwners
      .find(EditableListRow({ index: rowNumber - startRowIndex }))
...
export default {
  ...
  deleteOwnerWithServicePoint: (selectedDervicePoint) => {
    cy.then(() => tableWithOwners.find(MultiColumnListCell(selectedDervicePoint)).row()).then(
      (rowNumber) => {
  ...
  cancelAdding: (rowNumber = 0) => {
    cy.do(
      rootPaneset.find(Button({ id: `clickable-cancel-settings-owners-${rowNumber}` })).click(),
    );
    cy.expect(addButton.has({ disabled: false }));
  },
