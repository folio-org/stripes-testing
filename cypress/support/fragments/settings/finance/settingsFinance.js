...
export default {
  ...
  fillRequiredFields: (expenseClasses) => {
    cy.do([
      ...
      TextField({ placeholder: 'externalAccountNumberExt' }).fillIn(expenseClasses.externalAccountNumber),
      saveButton.click()
    ]);
  },
  ...
  editExpenseClass(expenseClass, oldExpenseClassName) {
    cy.do(MultiColumnListCell({ content: oldExpenseClassName }).perform(
      element => {
  ...
  checkExpenseClass: (expenseClass) => {
    cy.do(MultiColumnListCell({ content: expenseClass.name }).perform(
      element => {
        const rowNumber = element.parentElement.parentElement.getAttribute('data-row-index');
        const createdByAdmin = `${DateTools.getFormattedDateWithSlashes({ date: new Date() })} by folio-aqa, folio-aqa`;
