import { Heading, Link, Button, Section, PaneHeader, PaneContent, TextField, HTML, Select, KeyValue } from '../../../../../../interactors';
...
const maximumOverDueFineAlert = 'Maximum overdue fine must be greater than or equal to Overdue fine';
...
export default {
  waitLoading() {
    cy.expect([
      Link(mainTitle).exists(),
      Heading(mainTitle).exists(),
    ]);
  },
  ...
  fillOverdueFine(value) {
    cy.get('[name="overdueFine.quantity"]').invoke('val', '').clear()
      .type(value, { force : true })
      .blur();
  },

  fillMaxFine(value) {
    cy.get('[name="maxOverdueFine"]').focus().invoke('val', '').clear()
      .type(value, { force : true })
      .blur();
  },

  fillOverdueRecall(value) {
    cy.get('[name="overdueRecallFine.quantity"]').invoke('val', '').clear()
      .type(value, { force : true })
      .blur();
  },

  fillMaxRecall(value) {
    cy.get('[name="maxOverdueRecallFine"]').invoke('val', '').clear()
      .type(value, { force : true })
      .blur();
  },
  ...
  checkOverDueFineInCreating() {
    cy.do([
      ...
      overdueFineInput.focus(),
      overdueFineInput.blur(),
    ]);
  ...
  verifyCreatedFines([overdueFine, maxOverdueFine, recallFine, maxRecallFine]) {
    cy.expect([
      ...
      feeViewSection.find(KeyValue('Overdue recall fine', { value: `${recallFine} per day` })).exists(),
      feeViewSection.find(KeyValue('Maximum recall overdue fine', { value: maxRecallFine })).exists(),
    ]);
  },
