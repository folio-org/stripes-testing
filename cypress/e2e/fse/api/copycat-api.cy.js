...
describe('fse-copycat for production tenants', { retries: { runMode: 1 } }, () => {
  ...
  it(
    `TC195638 - Get all z3950 target profiles for ${Cypress.config('baseUrl')} - ${Cypress.env('OKAPI_TENANT')}`,
...
describe('fse-copycat for non-production tenants', () => {
  ...
  it(
    `TC195640 - Create and delete z3950 target profile for ${Cypress.config('baseUrl')} - ${Cypress.env('OKAPI_TENANT')}`,
