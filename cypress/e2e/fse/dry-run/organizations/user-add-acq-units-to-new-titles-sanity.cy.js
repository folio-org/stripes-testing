...
describe('Receiving', () => {
  describe('Acquisition units', () => {
        ...
        AcquisitionUnits.createAcquisitionUnitViaApi(secondAcquisitionUnit).then(
          (secondAUResponse) => {
            ...
            cy.getUsers({ limit: 1, query: `"username"="${user.username}"` }).then((users) => {
              ...
              AcquisitionUnits.assignUserViaApi(userData.id, firstAcquisitionUnit.id).then(() => {
                AcquisitionUnits.assignUserViaApi(userData.id, secondAcquisitionUnit.id);
              });

              Locations.getLocations({ limit: 1 }).then((locations) => {
    ...
    it(
      ...
      { tags: ['dryRun', 'thunderjet', 'C430259'] },
      () => {
