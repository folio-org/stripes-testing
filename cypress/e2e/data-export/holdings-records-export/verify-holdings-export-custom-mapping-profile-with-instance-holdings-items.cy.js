...
let loanTypeId;
let materialTypeId;
let isbnIdentifierTypeId;
let exportedFileName;
const holdingsIds = [];
const holdingsHrids = [];
const randomPostfix = getRandomPostfix();
const fileName = `AT_C423573_HoldingsUUIDs_${randomPostfix}.csv`;
const mappingProfileName = `AT_C423573_CustomMappingProfile_${randomPostfix}`;
const jobProfileName = `AT_C423573_CustomJobProfile_${randomPostfix} export job profile`;
const instanceOne = { title: `AT_C423573_MarcInstance_${randomPostfix}` };
const instanceTwo = { title: `AT_C423573_MarcInstance2_${randomPostfix}` };

const testData = {
  marcISBN: '9780123456789C423573',
  folioISBN: '9780987654321C423573',
  electronicAccess: {
    marcHolding: {
...
describe('Data Export', () => {
  describe('Holdings records export', () => {
        ...
        cy.getHoldingTypes({ limit: 1 })
          ...
          .then(() => {
            InventoryInstances.getIdentifierTypes({ query: 'name=="ISBN"' }).then(
              (identifierType) => {
                isbnIdentifierTypeId = identifierType.id;
              },
            );
          })
          .then(() => {
              ...
              cy.getInstanceById(marcInstanceId).then((instanceData) => {
                ...
                cy.createSimpleMarcHoldingsViaAPI(
                  ...
                  cy.getHoldings({
                    ...
                    const itemBarcode = `AT_C423573_MARC_${randomPostfix}`;
            ...
            cy.getInstanceTypes({ limit: 1 }).then((instanceTypes) => {
              InventoryInstances.createFolioInstanceViaApi({
                instance: {
                  ...
                  identifiers: [
                    {
                      identifierTypeId: isbnIdentifierTypeId,
                ...
                items: [
                  {
                    barcode: `AT_C423573_FOLIO_${randomPostfix}`,
                    status: { name: ITEM_STATUS_NAMES.AVAILABLE },
                    permanentLoanType: { id: loanTypeId },
                    materialType: { id: materialTypeId },
                  },
    ...
    it(
      'C423573 Verify export Inventory holdings records based on Custom mapping profile (Instance & Holdings & Item) (firebird)',
      { tags: ['criticalPath', 'firebird', 'C423573'] },
      () => {
        ...
        ExportFile.exportWithDefaultJobProfile(
          fileName,
          `AT_C423573_CustomJobProfile_${randomPostfix}`,
          'Holdings',
        );
        ...
        cy.wait('@getInfo', getLongDelay()).then(({ response }) => {
          ...
          // Step 5-6: Verify exported fields for MARC and FOLIO Holdings with associated Items and Instances
          const assertionsOnMarcFileContent = [
            {
              uuid: instanceOne.id,
              assertions: [
                (record) => expect(record.get('001')[0].value).to.eq(instanceOne.hrid),
                (record) => {
            ...
            },
            {
              uuid: instanceTwo.id,
              assertions: [
                (record) => expect(record.get('001')[0].value).to.eq(instanceTwo.hrid),
                (record) => {
                  expect(record.fields.find((f) => f[0] === '020')).to.deep.eq([
                    '020',
                    '  ',
