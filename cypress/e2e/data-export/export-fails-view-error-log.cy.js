    ...
    () => {
      ...
      cy.wait('@getJobInfo', getLongDelay()).then(({ response }) => {
        ...
        DataExportLogs.verifyErrorTextInErrorLogsPane(
          new RegExp(`${formattedDateUpToHours}.*ERROR Invalid UUID format: ${invalidUUIDFormat}`),
        );
        ...
        DataExportLogs.verifyErrorTextInErrorLogsPane(
          new RegExp(`${formattedDateUpToHours}.*ERROR UUID ${repeteableUUIDs} repeated 2 times.`),
        );
      });
