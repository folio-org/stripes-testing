...
const nameForCSVFile = `AT_C10968_file_${getRandomPostfix()}.csv`;
const instance = {
  title: `AT_C10968_FolioInstance_${getRandomPostfix()}`,
  id: '',
  hrid: '',
  physicalDescriptions: ['Test Physical Description'],
};
        ...
        cy.getInstanceTypes({ limit: 1 }).then((instanceTypeData) => {
          ...
          InventoryInstances.createFolioInstanceViaApi({
            instance: {
              ...
              physicalDescriptions: instance.physicalDescriptions,
            },
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getJobInfo', getLongDelay()).then(({ response }) => {
          ...
          // Step 5-6: Check exported MARC records - Verify physical description is mapped to 300$a with both indicators set to blank
          const assertionsOnMarcFileContent = [
            {
              uuid: instance.id,
              assertions: [
                (record) => {
                  const physicalDescriptionField = record.get('300')[0];

                  expect(physicalDescriptionField.ind1).to.eq(' ');
                  expect(physicalDescriptionField.ind2).to.eq(' ');
                  expect(physicalDescriptionField.subf[0][0]).to.eq('a');
                  expect(physicalDescriptionField.subf[0][1]).to.eq(
                    instance.physicalDescriptions[0],
                  );
                },
