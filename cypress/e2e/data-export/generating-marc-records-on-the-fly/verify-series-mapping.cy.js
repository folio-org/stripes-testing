...
const nameForCSVFile = `AT_C10969_file_${getRandomPostfix()}.csv`;
const instance = {
  title: `AT_C10969_FolioInstance_${getRandomPostfix()}`,
  id: '',
  hrid: '',
  series: [{ value: 'Test Series Statement' }],
};
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getJobInfo', getLongDelay()).then(({ response }) => {
          ...
          // Step 5-6: Check exported MARC records - Verify series is mapped to 490$a with first indicator 0 and second indicator blank
          const assertionsOnMarcFileContent = [
            {
              uuid: instance.id,
              assertions: [
                (record) => {
                  const seriesField = record.get('490')[0];

                  expect(seriesField.ind1).to.eq('0');
                  expect(seriesField.ind2).to.eq(' ');
                  expect(seriesField.subf[0][0]).to.eq('a');
                  expect(seriesField.subf[0][1]).to.eq(instance.series[0].value);
                },
