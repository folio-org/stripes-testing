...
const nameForCSVFile = `AT_C10972_file_${getRandomPostfix()}.csv`;
const instance = {
  title: `AT_C10972_FolioInstance_${getRandomPostfix()}`,
  id: '',
  hrid: '',
};
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getJobInfo', getLongDelay()).then(({ response }) => {
          ...
          // Step 5-6: Check exported MARC records - Verify Instance UUID is mapped to 999$i with both indicators f
          const assertionsOnMarcFileContent = [
            {
              uuid: instance.id,
              assertions: [
                (record) => {
                  const instanceUuidField = record.get('999')[0];

                  expect(instanceUuidField.ind1).to.eq('f');
                  expect(instanceUuidField.ind2).to.eq('f');
                  expect(instanceUuidField.subf[0][0]).to.eq('i');
                  expect(instanceUuidField.subf[0][1]).to.eq(instance.id);
                },
              ],
            },
          ];
