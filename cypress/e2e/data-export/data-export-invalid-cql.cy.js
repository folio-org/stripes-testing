...
describe('data-export', () => {
  beforeEach('create test data', () => {
    cy.createTempUser([
      ...
      .then(userProperties => {
        ...
        FileManager.createFile(`cypress/fixtures/${editedFileName}`, '(languages=="eng" and hello=="123") sortby title');
      });
  });
  ...
  it('C397323 Verify trigger Data export with invalid CQL (firebird)', { tags: [testTypes.criticalPath, devTeams.firebird, parallelization.nonParallel] }, () => {
    ...
    cy.wait('@getInfo', getLongDelay()).then((interception) => {
      ...
      DataExportResults.verifyFailedExportResultCells(resultFileName, recordsCount, jobId, user.username, 'instances', true);
    });
  });
});
