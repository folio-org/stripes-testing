...
describe('Bulk Edit - Logs', () => {
  before('create test data', () => {
    cy.createTempUser([
      ...
      .then(userProperties => {
        ...
        cy.getHoldings({ limit: 1, expandAll: true, query: `"instanceId"="${item.instanceId}"` }).then(holdings => {
  ...
  it('C402321 Verify "Suppress from discovery" option is set True in when Holdings are suppressed and associated Items are not (firebird)', { tags: [testTypes.criticalPath, devTeams.firebird] }, () => {
    ...
    BulkEditFiles.verifyMatchedResultFileContent(`*${matchedRecordsFileName}`, [item.instanceHRID], 'instanceHrid', true);

    BulkEditSearchPane.downloadFileWithProposedChanges();
    BulkEditFiles.verifyMatchedResultFileContent(previewOfProposedChangesFileName, [item.instanceHRID], 'instanceHrid', true);
