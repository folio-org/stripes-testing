...
const folioInstance = {
  title: `AT_C543812_FolioInstance_${getRandomPostfix()}`,
};
...
describe('Bulk-edit', () => {
  describe('In-app approach', () => {
    ...
    it(
      'C543812 Find and remove MARC field (533, 933, 999) with errors (MARC & FOLIO) (firebird)',
      { tags: ['criticalPath', 'firebird', 'C543812'] },
      () => {
        ...
        // Step 3: 533 $a Find, Remove field
        BulkEditActions.fillInTagAndIndicatorsAndSubfield('533', '\\', '\\', 'a');
        BulkEditActions.findAndRemoveFieldActionForMarc('Electronic reproduction.');
        BulkEditActions.verifyConfirmButtonDisabled(false);

        // Step 4: 933 $a Find, Remove field
        BulkEditActions.addNewBulkEditFilterStringForMarcInstance();
        BulkEditActions.fillInTagAndIndicatorsAndSubfield('933', '\\', '\\', 'a', 1);
        BulkEditActions.findAndRemoveFieldActionForMarc('electronic reproduction.', 1);
        BulkEditActions.verifyConfirmButtonDisabled(false);

        // Step 5: 933 $b Find, Remove field
        BulkEditActions.addNewBulkEditFilterStringForMarcInstance(1);
        BulkEditActions.fillInTagAndIndicatorsAndSubfield('933', '\\', '\\', 'b', 2);
        BulkEditActions.findAndRemoveFieldActionForMarc('Electronic reproduction.', 2);
        BulkEditActions.verifyConfirmButtonDisabled(false);

        // Step 6: 999 $f $f $i Find, Remove field
        BulkEditActions.addNewBulkEditFilterStringForMarcInstance(2);
        BulkEditActions.fillInTagAndIndicatorsAndSubfield('999', 'f', 'f', 'i', 3);
        BulkEditActions.findAndRemoveFieldActionForMarc(marcInstance.uuid, 3);
        BulkEditActions.verifyConfirmButtonDisabled(true);

        // Step 7: Info icon
        BulkEditActions.clickInfoIconNextToSubfieldAndVerifyText(3);

        // Step 8: Remove last row
        BulkEditActions.deleteRowInBulkEditMarcInstancesAccordion(3);
        BulkEditActions.verifyConfirmButtonDisabled(false);
        ...
        BulkEditFiles.verifyCSVFileRowsRecordsNumber(fileNames.previewCSV, 1);
