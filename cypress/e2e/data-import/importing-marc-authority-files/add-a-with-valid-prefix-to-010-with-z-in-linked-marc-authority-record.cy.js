...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376954_MarcAuthority_${randomPostfix}`;
    // no 'n' prefix — 010 starts with only $z, so $0 in bib = 001 HRID initially
    const naturalId = `${randomDigits}376954`;
    const zValue = `${randomDigits}0376954`;
    // valid $a prefix ('n') — bib $0 updates to LOC URL after import
    const validAValue = `n${randomDigits}1376954`;
    const sourceUrlPrefix = 'http://id.loc.gov/authorities/names/';
    const bibTitle = `AT_C376954_MarcBibInstance_${randomPostfix}`;

    const authorityUUIDsFileName = `AT_C376954_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376954 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376954 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376954 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcAuthFields = [
      // 010 has only $z — no $a, so bib $0 = 001 HRID before import
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376954_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376954_');
    ...
    it(
      'C376954 Add "$a" with valid prefix to "010" with "$z" in linked "MARC Authority" record when "001" = "$0" (spitfire)',
      { tags: ['extendedPath', 'spitfire', 'C376954'] },
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376954_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Prepend valid $a (n-prefix) before existing $z; bib $0 will update to LOC URL
          cy.then(() => {
            DataImport.editMarcFile(
              editedMarcFileName,
              editedMarcFileName,
              [`z${zValue}`],
              [`a${validAValue}z${zValue}`],
            );
          });
          ...
          // Steps 10-12: Authority 010 now has both valid $a and $z; Number of titles maintained
          ...
          MarcAuthority.contains(validAValue);
          MarcAuthority.contains(zValue);

          // Steps 13-16: Bib 711 $0 updated to LOC URL based on valid $a value
