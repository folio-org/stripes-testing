...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376950_MarcAuthority_${randomPostfix} 1950-2022 SubCAuth`;
    const authorityField100Content = `$a AT_C376950_MarcAuthority_${randomPostfix} $d 1950-2022 $c SubCAuth`;
    const naturalId = `${randomDigits}376950`;
    const zValue = `${randomDigits}0376950`;
    const bibTitle = `AT_C376950_MarcBibInstance_${randomPostfix}`;
    const bibField800OriginalContent =
      '$a AT_C376950_Bib800 $d 1950- $t Inspector Banks series $v 24. $y 2023 $8 800';

    const csvFile = `AT_C376950_authorityUUIDs_${randomPostfix}.csv`;
    const exportedMarcFile = `AT_C376950_exportedMarcFile_${randomPostfix}.mrc`;
    const editedMarcFile = `AT_C376950_editedMarcFile_${randomPostfix}.mrc`;

    const mappingProfile = { name: `AT_C376950_MappingProfile_${randomPostfix}` };
    const actionProfile = {
      name: `AT_C376950_ActionProfile_${randomPostfix}`,
      action: 'UPDATE',
      folioRecordType: EXISTING_RECORD_NAMES.MARC_AUTHORITY,
    };
    const matchProfile = {
      profileName: `AT_C376950_MatchProfile_${randomPostfix}`,
    ...
    const jobProfile = {
      ...
      profileName: `AT_C376950 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      acceptedType: ACCEPTED_DATA_TYPE_NAMES.MARC,
    };

    // The authority record does NOT have a "010" field yet - it's added by the reimport
    ...
    const bibField800AfterLinking = {
      ...
      alphaContr: `$a AT_C376950_MarcAuthority_${randomPostfix} $d 1950-2022 $c SubCAuth`,
    ...
    before('Create test data, job profiles, login', () => {
      ...
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376950_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376950_');
      ...
      cy.then(() => {
        ...
        .then(() => {
          // Add a new "010" field with only "$z" (no "$a") - the field-length change requires
          // the marcjs-based editor, not the plain text-replace one
          DataImport.editMarcFieldsInAllRecords(exportedMarcFile, editedMarcFile, {
            addFields: [{ tag: '010', indicators: ['\\', '\\'], content: `$z ${zValue}` }],
          });
        })
        .then(() => {
          cy.createTempUser([
    ...
    it(
      'C376950 Add new "010" with "$z" field in linked "MARC Authority" record when "001" = "$0" (promin)',
      { tags: ['extendedPath', 'promin', 'C376950'] },
      () => {
        ...
        // Step 12: detail view shows the newly added "010" field
