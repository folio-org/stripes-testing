...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376951_MarcAuthority_${randomPostfix}`;
    // no 'n' prefix — 010 has only $z, so $0 in bib = 001 HRID (this value)
    const naturalId = `${randomDigits}376951`;
    const zValue = `${randomDigits}0376951`;
    const updatedZValue = `${randomDigits}1376951`;
    const bibTitle = `AT_C376951_MarcBibInstance_${randomPostfix}`;

    const authorityUUIDsFileName = `AT_C376951_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376951 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376951 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376951 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcAuthFields = [
      // 010 has only $z — no $a, so bib $0 = 001 HRID instead of a LOC URL
    ...
    const marcBibFields = [
      ...
      { tag: '800', content: '$a Field800', indicators: ['1', '\\'] },
    ];
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376951_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376951_');
    ...
    it(
      'C376951 Update "010 $z" in linked "MARC Authority" record when "001" = "$0" (spitfire)',
      { tags: ['extendedPath', 'spitfire', 'C376951'] },
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376951_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Update the $z value; $a absent so bib $0 stays as 001 HRID
          cy.then(() => {
          ...
          // Steps 10-12: Authority 010 shows updated $z; Number of titles maintained
          ...
          MarcAuthority.contains(updatedZValue);
          MarcAuthority.notContains(zValue);

          // Steps 13-16: Bib 800 is still linked; $0 unchanged = authority 001 HRID
