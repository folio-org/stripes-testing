...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376955_MarcAuthority_${randomPostfix}`;
    const naturalId = `${randomDigits}376955`;
    // no letter prefix — 010 $a is already invalid, so $0 in bib = 001 HRID
    const originalAValue = `xyxyxy${randomDigits}0376955`;
    // updated invalid value: still no letter prefix → bib $0 stays as 001 HRID
    const updatedAValue = `${randomDigits}1376955`;
    const bibTitle = `AT_C376955_MarcBibInstance_${randomPostfix}`;

    const authorityUUIDsFileName = `AT_C376955_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376955 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376955 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376955 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcAuthFields = [
      // 010 $a has no letter prefix = invalid; bib $0 = 001 HRID
      { tag: '010', content: `$a ${originalAValue}`, indicators: ['\\', '\\'] },
      { tag: '130', content: `$a ${authorityHeading}`, indicators: ['\\', '\\'] },
    ];

    const marcBibFields = [
      { tag: '008', content: QuickMarcEditor.valid008ValuesInstance },
      { tag: '245', content: `$a ${bibTitle}`, indicators: ['1', '1'] },
      { tag: '630', content: '$a Field630', indicators: ['0', '0'] },
    ];
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376955_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376955_');
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376955_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Update $a to different invalid value (still no letter prefix)
          cy.then(() => {
          ...
          // Steps 10-12: Authority 010 shows updated $a; Number of titles maintained
          ...
          MarcAuthority.contains(updatedAValue);
          MarcAuthority.notContains(originalAValue);

          // Steps 13-16: Bib 630 $0 unchanged = 001 HRID ($a still invalid, no LOC URL update)
