...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376942_MarcAuthority_${randomPostfix}`;
    const naturalId = `n${randomDigits}376942`;
    // $z is the remaining subfield after $a is deleted
    const zValue = `${randomDigits}0376942`;
    const bibTitle = `AT_C376942_MarcBibInstance_${randomPostfix}`;

    const authorityUUIDsFileName = `AT_C376942_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376942 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376942 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376942 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcAuthFields = [
      // 010 has both $a and $z; test deletes $a leaving only $z
      { tag: '010', content: `$a ${naturalId} $z ${zValue}`, indicators: ['\\', '\\'] },
      { tag: '130', content: `$a ${authorityHeading}`, indicators: ['\\', '0'] },
    ];

    const marcBibFields = [
      { tag: '008', content: QuickMarcEditor.valid008ValuesInstance },
      { tag: '245', content: `$a ${bibTitle}`, indicators: ['1', '1'] },
      { tag: '830', content: '$a Field830', indicators: ['\\', '0'] },
    ];
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376942_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376942_');
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376942_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Delete $a from 010 — search for $a subfield code + value, replace with empty
          cy.then(() => {
          ...
          // Steps 10-12: Authority 010 shows only $z (no $a); Number of titles maintained
          ...
          // Steps 13-15: Bib 830 is still linked; $0 replaced by authority 001 HRID (no valid $a)
