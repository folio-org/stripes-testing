...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C374180_MarcAuthority_${randomPostfix}`;
    const naturalId = `n${randomDigits}374180`;
    const updatedNaturalId = `n${randomDigits}1374180`;
    const bibTitle = `AT_C374180_MarcBibInstance_${randomPostfix}`;
    const sourceUrlPrefix = 'http://id.loc.gov/authorities/names/';

    const authorityUUIDsFileName = `AT_C374180_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C374180 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C374180 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C374180 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C374180_');
      InventoryInstances.deleteInstanceByTitleViaApi('C374180_');
    ...
    it(
      'C374180 Update "010 $a" with valid value in linked "MARC Authority" record when "010" = "$0" (spitfire)',
      { tags: ['extendedPath', 'spitfire', 'C374180'] },
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C374180_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Update 010 $a value from naturalId → updatedNaturalId
          cy.then(() => {
          ...
          // Steps 10-12: Verify authority has updated 010 $a and "Number of titles" is maintained
          ...
          // Steps 13-15: Verify bib 700 field is still linked and $0 is updated
          ...
          QuickMarcEditor.verifyTagFieldAfterLinkingByTag(
            ...
            `$a ${authorityHeading}`,
            '',
            `$0 ${sourceUrlPrefix}${updatedNaturalId}`,
            '',
          );
        });
      },
