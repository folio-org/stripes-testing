...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376939_MarcAuthority_${randomPostfix}`;
    const naturalId = `n${randomDigits}376939`;
    // $z value appended to 010 — $a stays unchanged so bib $0 is NOT updated
    const zValue = `${randomDigits}0376939`;
    const bibTitle = `AT_C376939_MarcBibInstance_${randomPostfix}`;
    const sourceUrlPrefix = 'http://id.loc.gov/authorities/names/';

    const authorityUUIDsFileName = `AT_C376939_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376939 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376939 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376939 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcBibFields = [
      ...
      { tag: '811', content: '$a Field811', indicators: ['2', '\\'] },
    ];
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376939_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376939_');
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376939_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Append $z subfield to 010 after $a value; 0x1f is the MARC subfield delimiter
          cy.then(() => {
          ...
          // Steps 10-12: Authority shows $a (unchanged) and new $z; Number of titles still 1
          ...
          MarcAuthority.contains(naturalId);
          MarcAuthority.contains(zValue);

          // Steps 13-15: Bib 811 field is still linked; $0 is NOT updated (010 $a unchanged)
