...
describe('Data Import', () => {
  describe('Importing MARC Authority files', () => {
    ...
    const authorityHeading = `AT_C376938_MarcAuthority_${randomPostfix}`;
    const naturalId = `n${randomDigits}376938`;
    // invalid: no letter prefix, so FOLIO falls back to 001 HRID for $0 instead of a LOC URL
    const updatedNaturalId = `${randomDigits}1376938`;
    const bibTitle = `AT_C376938_MarcBibInstance_${randomPostfix}`;

    const authorityUUIDsFileName = `AT_C376938_authorityUUIDs_${randomPostfix}.csv`;
    const defaultAuthorityExportProfile = 'Default authority';

    const mappingProfile = {
      name: `AT_C376938 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    };
    const actionProfile = {
      name: `AT_C376938 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
      action: 'UPDATE',
    ...
    const matchProfile = {
      profileName: `AT_C376938 Update MARC authority records by matching 999 ff $s ${randomPostfix}`,
    ...
    const marcBibFields = [
      ...
      { tag: '700', content: '$a Field700', indicators: ['1', '\\'] },
    ];
    ...
    before('Create test data, job profiles, login', () => {
      cy.getAdminToken();
      MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C376938_');
      InventoryInstances.deleteInstanceByTitleViaApi('C376938_');
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getExportInfo', getLongDelay()).then(({ response }) => {
          ...
          editedMarcFileName = `C376938_edited_${randomPostfix}.mrc`;
          ...
          // Steps 5-7: Replace valid naturalId with invalid value (no letter prefix)
          cy.then(() => {
          ...
          // Steps 10-12: Verify authority has the invalid 010 $a and "Number of titles" is maintained
          ...
          MarcAuthority.contains(updatedNaturalId);
          MarcAuthority.notContains(naturalId);

          // Steps 13-15: Verify bib 700 field is still linked ($0 replaced by authority 001 HRID)
          ...
          // $0 falls back to authority HRID (001 value) since invalid naturalId has no letter prefix
