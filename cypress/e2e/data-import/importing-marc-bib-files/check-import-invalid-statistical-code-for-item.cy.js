...
describe('Data Import', () => {
  describe('Importing MARC Bib files', () => {
    ...
    const testData = {
      marcFile: {
        marc: 'marcBibFileForC1395028.mrc',
        fileName: `testMarcFileC1395028.${randomPostfix}.mrc`,
      },
      instanceTitle: 'AT_C1395028_MarcBibInstance',
      errorMessage: 'Provided Statistical code(s) are not a valid values:',
    };

    const mappingProfiles = {
      instance: { name: `AT_C1395028 FMP Instance ${randomPostfix}` },
      holdings: { name: `AT_C1395028 FMP Holdings ${randomPostfix}` },
      item: { name: `AT_C1395028 FMP Item ${randomPostfix}` },
    };

    const actionProfiles = {
      instance: {
        name: `AT_C1395028 AP Instance ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'INSTANCE',
      },
      holdings: {
        name: `AT_C1395028 AP Holdings ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'HOLDINGS',
      },
      item: {
        name: `AT_C1395028 AP Item ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'ITEM',
      },
    };

    const jobProfile = { name: `AT_C1395028 JP ${randomPostfix}` };
    ...
    before('Create profiles and user', () => {
      ...
      cy.then(() => {
        ...
        // Step 3: Create Holdings mapping profile - permanent location only, no statistical codes
        FieldMappingProfiles.createMappingProfileViaApi({
          profile: {
    ...
    it(
      ...
      () => {
        ...
        // Step 13: Open file details and verify CREATED for Instance and Holdings, NO_ACTION for Item
        ...
        // Step 14: Open JSON screen and verify error message in Item tab
