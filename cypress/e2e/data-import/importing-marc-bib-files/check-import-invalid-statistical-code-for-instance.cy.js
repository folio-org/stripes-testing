...
describe('Data Import', () => {
  describe('Importing MARC Bib files', () => {
    ...
    const testData = {
      marcFile: {
        marc: 'marcBibFileForC1395026.mrc',
        fileName: `testMarcFileC1395026.${randomPostfix}.mrc`,
      },
      instanceTitle: 'AT_C1395026_MarcBibInstance',
      errorMessage: 'Provided Statistical code(s) are not a valid values:',
    };

    const mappingProfiles = {
      instance: { name: `AT_C1395026 FMP Instance ${randomPostfix}` },
      holdings: { name: `AT_C1395026 FMP Holdings ${randomPostfix}` },
      item: { name: `AT_C1395026 FMP Item ${randomPostfix}` },
    };

    const actionProfiles = {
      instance: {
        name: `AT_C1395026 AP Instance ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'INSTANCE',
      },
      holdings: {
        name: `AT_C1395026 AP Holdings ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'HOLDINGS',
      },
      item: {
        name: `AT_C1395026 AP Item ${randomPostfix}`,
        action: 'CREATE',
        folioRecordType: 'ITEM',
      },
    };

    const jobProfile = { name: `AT_C1395026 JP ${randomPostfix}` };
    ...
    before('Create profiles and user', () => {
      ...
      cy.then(() => {
        // Steps 1-2: Create Instance mapping profile with invalid statistical code
    ...
    after('Delete test data', () => {
      ...
      InventoryInstances.deleteFullInstancesByTitleViaApi('C1395026_');
    });

    it(
      'C1395026 Check the import of invalid statistical code for instance (folijet)',
      { tags: ['extendedPath', 'folijet', 'C1395026'] },
      () => {
        ...
        // Step 13: Open file details and verify NO_ACTION in Instance column
        ...
        // Step 14: Open JSON screen and verify error message in Instance tab
