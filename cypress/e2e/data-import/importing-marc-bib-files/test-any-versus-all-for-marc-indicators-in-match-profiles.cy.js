...
describe('ui-data-import', () => {
  ...
  const collectionOfMatchProfiles = [
    {
      matchProfile: { profileName: `C17036 FAIL match profile_${getRandomPostfix()}`,
  ...
  const collectionOfMappingAndActionProfilesForUpdate = [
    {
      mappingProfile: { name: `C17036 item update mapping profile_${getRandomPostfix()}`,
        typeValue: FOLIO_RECORD_TYPE.ITEM,
  ...
  it('C17036 Test Any versus All for MARC indicators in match profiles (folijet)',
    { tags: [TestTypes.criticalPath, DevTeams.folijet] }, () => {
      // change file for adding random barcode
      DataImport.editMarcFile(filePathToUpload, editedMarcFileName, ['testBarcode'], [itemBarcode]);
      ...
      NewFieldMappingProfile.fillItemIdentifier(collectionOfMappingAndActionProfilesForUpdate[0].mappingProfile.itemIdentifier);
      FieldMappingProfiles.saveProfile();
      ...
      NewJobProfile.linkMatchAndActionProfiles(collectionOfMatchProfiles[1].matchProfile.profileName, collectionOfMappingAndActionProfilesForUpdate[0].actionProfile.name);
      NewJobProfile.saveAndClose();
      JobProfiles.checkJobProfilePresented(jobProfileForCreate.profileName);
