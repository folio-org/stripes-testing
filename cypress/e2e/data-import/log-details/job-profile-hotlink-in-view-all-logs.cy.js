...
describe('Data Import', () => {
  describe('Log details', () => {
    ...
    const mappingProfile = {
      name: `AT_C380636_MappingProfile_${randomPostfix}`,
      typeValue: FOLIO_RECORD_TYPE.INSTANCE,
    };
    const actionProfile = {
      name: `AT_C380636_ActionProfile_${randomPostfix}`,
      action: 'CREATE',
      folioRecordType: EXISTING_RECORD_NAMES.INSTANCE,
    };
    const jobProfile = { profileName: `AT_C380636_JobProfile_${randomPostfix}` };
    const importedFileName = `AT_C380636_importedFile_${randomPostfix}.mrc`;
    ...
    it(
      'C380636 View all: Verify that job profile name in logs is a hotlink to the job profile details (promin)',
      { tags: ['extendedPath', 'promin', 'C380636'] },
      () => {
        // Steps 1-4: Open "View all logs"; click the job profile hotlink - details view opens in the 4th pane
        Logs.openViewAllLogs();
        LogsViewAll.viewAllIsOpened();
        LogsViewAll.openJobProfile(jobProfile.profileName);
        JobProfileView.verifyJobProfileOpened();
        JobProfileView.verifyJobProfileName(jobProfile.profileName);

        // Step 5: Delete the job profile - no longer present in the job profiles list
        ...
        // Step 6: Intercept the job profile request (stand-in for inspecting the Network tab)
        cy.intercept('GET', `**/data-import-profiles/jobProfiles/${jobProfileId}*`).as(
          'getDeletedJobProfile',
        );

        // Step 7: Repeat steps 1-4 - same hotlink text, but the pane now shows the deleted state, and 404s
