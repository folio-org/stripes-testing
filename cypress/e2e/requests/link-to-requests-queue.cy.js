...
describe('Requests', () => {
  ...
  const requestPolicyBody = {
    ...
    name: getTestEntityValue('RequestPolicyC749'),
    id: uuid(),
  };
  ...
  before('Create test data', () => {
    ...
    cy.wrap(null).then(() => {
      ...
      // Check out item first to allow recall requests
  ...
  it(
    ...
    () => {
      ...
      cy.getCancellationReasonsApi({ limit: 1 }).then((cancellationReasons) => {
        ...
        const requestId = testData.requestIds[1];

        Requests.getRequestApi({ query: `(id=="${requestId}")` }).then((requestResponse) => {
          const requestToCancel = {
            ...requestResponse[0],
            status: 'Closed - Cancelled',
            cancelledByUserId: testData.requesters[2].userId,
