...
describe('Notes', () => {
  ...
  it(
    ...
    () => {
      ...
      cy.intercept('GET', '**/note-types?query=cql.allRecords=1*').as('getNoteTypes');
      NoteTypes.clickGeneralButton();
      cy.wait('@getNoteTypes').then((interception) => {
        const responseNotes = interception.response.body.noteTypes;
        const foundNoteType = responseNotes.find((note) => note.name === testData.noteType);
