...
describe('ui-eholdings: Search titles', () => {
  ...
  it('C684 Title Search: Search titles for chemical engineering. Then filter results to journals. (spitfire)', { tags: [TestTypes.smoke, DevTeams.spitfire, Features.eHoldings] }, () => {
    cy.login(testData.C684UserProperties.username, testData.C684UserProperties.password, { path: TopMenu.eholdingsPath, waiter: EHoldingsTitlesSearch.waitLoading });
  ...
  it('C9240 Selected Title+Package: Add a value to a custom label (spitfire)', { tags: [TestTypes.criticalPath, DevTeams.spitfire] }, () => {
    cy.login(testData.C9240UserProperties.username, testData.C9240UserProperties.password, { path: TopMenu.eholdingsPath, waiter: EHoldingsTitlesSearch.waitLoading });
    EHoldingSearch.switchToTitles();
