...
describe('eHoldings', () => {
  describe('Package', () => {
    ...
    const testData = {
      ...
      fileName: `C353945autoTestFile${postfix}.csv`,
      fileMask: '*_package.csv',
      packageData: `C353945_package_data_${postfix}.csv`,
      titleData: `C353945_title_data_${postfix}.csv`,
    ...
    it(
      ...
      () => {
        ...
        // Step 5: Click on the "Package" record
        ...
        // Step 8: Package fields multi-select shows every available option
        EHoldingsPackageView.verifySelectedPackageFieldsOptions();

        // Step 9: select every Package field - Export button becomes enabled
        ...
        // Step 10: Title fields multi-select shows every available option
        EHoldingsPackageView.verifySelectedTitleFieldsOptions();

        // Step 11: select every Title field
        ...
        EHoldingsPackageView.getJobIDFromCalloutMessage().then((jobId) => {
          ...
          FileManager.convertCsvToJson(testData.packageData).then((data) => {
            ...
            // A single assertion over all headers at once, instead of looping cy.expect/cy.wait
            // per header - looping real cy commands here just adds command-log overhead (this
            // test already has ~55 UI field-selection commands) without checking anything extra
