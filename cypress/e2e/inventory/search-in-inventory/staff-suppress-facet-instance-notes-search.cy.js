...
describe('Inventory', () => {
  describe('Search in Inventory', () => {
    describe('Filters', () => {
      ...
      const noteText = `AT_C446030_Instance_note_${randomPostfix}`;
      const instanceTitlePrefix = `AT_C446030_Instance_${randomPostfix}`;
      const accordionName = 'Staff suppress';
      const searchOption = searchInstancesOptions[9]; // Instance notes (all)
      ...
      before('Create test data and login', () => {
        cy.getAdminToken();
        InventoryInstances.deleteInstanceByTitleViaApi('AT_C446030_Instance');

        cy.then(() => {
          InstanceNoteTypes.getInstanceNoteTypesViaApi({
            limit: 1,
            query: `name=="${NOTE_TYPES.GENERAL}"`,
          }).then((response) => {
            ...
            cy.getInstanceTypes({ limit: 1, query: 'source=rdacontent' }).then((instanceTypes) => {
              ...
              [1, 2].forEach((folioIndex) => {
                ...
                const instance = {
                  ...
                  notes: [
                    {
                      instanceNoteTypeId: generalNoteType.id,
                      note: noteText,
                      staffOnly: false,
                    },
                  ],
      ...
      it(
        'C446030 Staff suppress facet is off by default when user has permission to use facet (search by "Instance notes (all)") (spitfire)',
        { tags: ['extendedPath', 'spitfire', 'C446030'] },
        () => {
          ...
          InventorySearchAndFilter.searchByParameter(searchOption, noteText);
