...
describe('Inventory', () => {
  describe('Search in Inventory', () => {
    describe('Filters', () => {
      ...
      const administrativeNoteText = `AT_C446031_Administrative_note_${randomPostfix}`;
      const instanceTitlePrefix = `AT_C446031_Instance_${randomPostfix}`;
      const accordionName = 'Staff suppress';
      const searchOption = searchInstancesOptions[10]; // Instance administrative notes
      ...
      before('Create test data and login', () => {
        cy.getAdminToken();
        InventoryInstances.deleteInstanceByTitleViaApi('AT_C446031_Instance');
            ...
            [3, 4].forEach((marcIndex) => {
              ...
              cy.createMarcBibliographicViaAPI(
                ...
                cy.getInstanceById(instanceId).then((body) => {
                  body.administrativeNotes = [administrativeNoteText];
                  if (isStaffSuppressed) {
      ...
      it(
        'C446031 Staff suppress facet is off by default when user has permission to use facet (search by "Instance administrative notes") (spitfire)',
        { tags: ['extendedPath', 'spitfire', 'C446031'] },
        () => {
          ...
          InventorySearchAndFilter.searchByParameter(searchOption, administrativeNoteText);
