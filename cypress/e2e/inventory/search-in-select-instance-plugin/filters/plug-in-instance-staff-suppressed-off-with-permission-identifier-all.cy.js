...
describe('Inventory', () => {
  describe('Search in "Select instance" plugin', () => {
    describe('Filters', () => {
      ...
      const instanceTitlePrefix = `AT_C446104_Instance_${randomPostfix}`;
      const identifierValue = `AT_C446104_Identifier_${randomPostfix}`;
      const organization = NewOrganization.getDefaultOrganization();
      organization.name = `AT_C446104_Org_${randomPostfix}`;
      ...
      before('Create users, data', () => {
        ...
        cy.then(() => {
          InventoryInstances.deleteInstanceByTitleViaApi('AT_C446104');

          cy.getInstanceTypes({ limit: 1, query: 'source=rdacontent' }).then((instanceTypes) => {
            instanceTypeId = instanceTypes[0].id;
          });
          cy.getInstanceIdentifierTypes({ limit: 3, query: 'source="folio"' }).then(() => {
            identifierTypeIds.push(...Cypress.env('identifierTypes').map((type) => type.id));
          });
                    ...
                    identifiers: [
                      {
                        ...
                        identifierTypeId: identifierTypeIds[index],
                      },
                    ],
          ...
          .then(() => {
            cy.createTempUser([
              ...
              SelectInstanceModal.chooseSearchOption(identifierAllSearchOption);
              SelectInstanceModal.checkSearchOptionSelected(identifierAllSearchOption);
            });
          });
      });
