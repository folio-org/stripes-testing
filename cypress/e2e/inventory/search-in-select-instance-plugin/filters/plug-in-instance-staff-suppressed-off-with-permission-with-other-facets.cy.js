...
describe('Inventory', () => {
  describe('Search in "Select instance" plugin', () => {
    describe('Filters', () => {
      ...
      const instanceTitlePrefix = `AT_C446113_Instance_${randomPostfix}`;
      const organization = NewOrganization.getDefaultOrganization();
      organization.name = `AT_C446113_Org_${randomPostfix}`;
      ...
      const marcInstanceTitles = instanceTitles.filter(
        (_, index) => instancesData[index].source === INSTANCE_SOURCE_NAMES.MARC,
      );
      ...
      before('Create users, data', () => {
        ...
        cy.then(() => {
          InventoryInstances.deleteInstanceByTitleViaApi('AT_C446113');
      ...
      it(
        'C446113 Find Instance plugin | Staff suppress facet is off by default when other facets are applied and when user has permission to use facet (promin)',
        { tags: ['extendedPath', 'promin', 'C446113'] },
        () => {
          cy.ifConsortia(true, () => {
          ...
          InventorySearchAndFilter.verifyNumberOfSearchResults(marcInstanceTitles.length);
          marcInstanceTitles.forEach((title) => {
            InventorySearchAndFilter.verifySearchResult(title);
          });
