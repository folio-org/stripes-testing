...
describe('Inventory', () => {
  describe('Search in "Select instance" plugin', () => {
    ...
    const titlePrefix = `AT_C543877_FolioInstance_${randomPostfix}`;
    const contributorPrefix = `AT_C543877_Contributor_${randomPostfix}`;
    const testData = {
    ...
    before('Create test data, user', () => {
      cy.getAdminToken();
      InventoryInstances.deleteInstanceByTitleViaApi('C543877_');
    ...
    it(
      ...
      () => {
        ...
        // Step 4: Search again; verify Contributors sort is restored
        SelectInstanceModal.searchByName(titlePrefix);
        InventorySearchAndFilter.verifySearchResult(instancesData[0].title);
        InventoryInstances.checkResultListSortedByColumn(1);
        InventoryInstances.checkColumnHeaderSort(INVENTORY_DEFAULT_SORT_OPTIONS.CONTRIBUTORS);
