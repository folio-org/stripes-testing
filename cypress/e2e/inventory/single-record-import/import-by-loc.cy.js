...
const expectedFieldC490901 = { tag: '035', position: 4, content: '$a 21621300' };
...
describe('Inventory', () => {
  describe('Single record import', () => {
    before('Create test user and login', () => {
      cy.getAdminToken();
      [instanceTitlePartC490900, instanceTitlePartC490901, instanceTitlePartC490902].forEach(
        (title) => {
    ...
    it(
      ...
      () => {
        ...
        InventoryInstance.getId().then((id) => createdInstaceIds.push(id));
        InventoryInstance.editMarcBibliographicRecord();
        QuickMarcEditor.verifyTagValue(expectedFieldC490901.position, expectedFieldC490901.tag);
        QuickMarcEditor.checkContent(expectedFieldC490901.content, expectedFieldC490901.position);
        QuickMarcEditor.verifyNoDuplicatedFieldsWithTag(expectedFieldC490901.tag);
      },
    );
