...
describe('Inventory', () => {
  describe('Item', () => {
    const itemData = {
      ...
      instanceTitle: `AT_C633_Instance_${getRandomPostfix()}`,
      instances: [
        {
          title: `AT_C633_instance ${getRandomPostfix()}`,
        },
        {
          title: `AT_C633_instance ${getRandomPostfix()}`,
        },
    ...
    it(
      'C633 Locations --> Permanent Location --> (Validate in Settings) (Promin) (TaaS)',
      { tags: ['extendedPath', 'promin', 'C633'] },
      () => {
        ...
        ItemRecordEdit.openPermanentLocation();
        [...Array(3)].forEach((_, index) => {
          ItemRecordEdit.verifyPermanentLocationItemExists(itemData.instances[index].location.name);
        });
