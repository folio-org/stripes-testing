...
describe('Inventory', () => {
  describe('Instance', () => {
    ...
    const testData = {
      instanceTitle: `AT_C523735_FolioInstance_${randomPostfix}`,
    };
    ...
    it(
      'C523735 Check that "Save & close" button disabled after clicking it one time (folijet)',
      { tags: ['extendedPath', 'folijet', 'C523735'] },
      () => {
        ...
        // Step 3: Add "Test" to Resource title field, click Save & close;
        // verify button becomes disabled immediately, form closes, changes saved
        InstanceRecordEdit.editResourceTitle(updatedTitle);
        InstanceRecordEdit.clickSaveAndCloseButton();
        InstanceRecordEdit.checkSaveAndCloseButtonDisabledOrNotShown();
        InventoryInstance.waitLoading();
