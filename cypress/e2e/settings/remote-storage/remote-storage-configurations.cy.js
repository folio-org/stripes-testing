...
describe('remote-storage-configuration', () => {
  ...
  it(
    'C343288 Data synchronization settings section must be present only for Dematic StagingDirector provider (vega)',
    { tags: ['criticalPath', 'vega', 'C343288'] },
    () => {
  ...
  it(
    'C343287 Data synchronization settings field must be undefined for any provider except Dematic StagingDirector (vega)',
    { tags: ['criticalPath', 'vega', 'C343287'] },
    () => {
      const name = `AutotestConfigurationName${getRandomPostfix()}`;
