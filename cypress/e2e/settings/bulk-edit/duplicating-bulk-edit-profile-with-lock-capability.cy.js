...
const createUnlockedProfileBody = () => {
  return createBulkEditProfileBody({
    name: `AT_C889724 unlocked instances bulk edit profile ${getRandomPostfix()}`,
    description: 'Unlocked instances profile description for duplication test with lock capability',
    locked: false,
    entityType: 'INSTANCE',
    ruleDetails: [
      createAdminNoteRule(
...
const createLockedProfileBody = () => {
  return createBulkEditProfileBody({
    name: `AT_C889724 locked instances bulk edit profile ${getRandomPostfix()}`,
    description: 'Locked instances profile description for duplication test with lock capability',
    locked: true,
    entityType: 'INSTANCE',
    ruleDetails: [
...
const testData = {
  unlockedProfileDescription:
    'Unlocked instances profile description for duplication test with lock capability',
  lockedProfileDescription:
    'Locked instances profile description for duplication test with lock capability',
  ...
  newAdministrativeNote: 'Modified administrative note for duplicated profile with lock',
  newAdministrativeNoteForLockedDuplicate:
    'Modified administrative note for locked profile duplicate with lock capability',
};
  ...
  describe('Profiles', () => {
    ...
    it.skip(
      ...
      () => {
        ...
        // Step 6: Check "Lock profile" checkbox and make any changes under "Bulk edits" accordion
        ...
        // Step 10: Click "X" button, open locked bulk edit profile and click "Actions" menu button
        ...
        // Step 11: Click "Duplicate" button on locked profile
        InstancesBulkEditProfileView.selectDuplicateProfile();
        InstancesBulkEditProfileForm.waitLoading();
        InstancesBulkEditProfileForm.verifyFormElements('New FOLIO instances bulk edit profile');

        // Step 12: Verify elements under "Summary" accordion for locked profile duplicate
        InstancesBulkEditProfileForm.verifySummaryAccordionElements(false);
        InstancesBulkEditProfileForm.verifyMetadataSectionExists();

        // Step 13: Verify profile name prefixed with "Copy of"
        ...
        // Step 15: Edit fields except "Lock profile" checkbox
        duplicatedLockedProfileName = `AT_C889724 Copy of locked profile ${getRandomPostfix()}`;
        const newDescription =
          'Modified description for locked profile duplicate with lock capability';
        ...
        InstancesBulkEditProfileForm.verifyLockProfileCheckboxChecked(false, false);
        InstancesBulkEditProfileForm.verifySaveButtonDisabled(false);
        ...
        // Step 17: Verify row with created duplicate profile in the table
        InstancesBulkEditProfilesPane.verifyProfileInTable(
        ...
        // Step 18: Click on the row with created duplicate of locked bulk edit profile
