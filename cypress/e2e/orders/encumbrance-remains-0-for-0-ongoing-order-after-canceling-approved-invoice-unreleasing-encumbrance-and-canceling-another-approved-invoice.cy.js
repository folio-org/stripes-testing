...
describe('Orders', () => {
  describe('Order lines', () => {
    ...
    const createAndApproveInvoices = () => {
      return createAndApproveInvoice({ invoiceKey: 'first', subTotal: 5 }).then(() => {
        return createAndApproveInvoice({ invoiceKey: 'second', subTotal: 10 });
      });
    };
    ...
    it(
      'C844253 Encumbrance remains 0 for an $0 Ongoing order after canceling an approved invoice, unreleasing encumbrance, and canceling another approved invoice (release encumbrance = true) (thunderjet)',
      { tags: ['criticalPath', 'thunderjet', 'C844253', 'nonParallel'] },
      () => {
        ...
        // Step 6: Check the current budget of Fund A
        Funds.closeBudgetTransactionApp(testData.budget.name);
        BudgetDetails.waitLoading();
        BudgetDetails.checkBudgetDetails({
          summary: [
        ...
        // Step 8: Open the encumbrance from the invoice line of Invoice #1
        ...
        // Step 9: Check the current budget of Fund A
        Funds.closeBudgetTransactionApp(testData.budget.name);
        BudgetDetails.waitLoading();
        BudgetDetails.checkBudgetDetails({
          summary: [
