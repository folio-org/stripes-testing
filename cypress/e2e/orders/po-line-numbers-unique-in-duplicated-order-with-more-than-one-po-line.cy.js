...
describe('Orders', () => {
  ...
  const getPoLineData = () => ({
    itemDetails: { title: `AT_C397989_POLine_${getRandomPostfix()}` },
    poLineDetails: {
  ...
  after('Delete test data', () => {
    cy.getAdminToken().then(() => {
      ...
      OrderLinesLimit.setPOLLimitViaApi(testData.initialPoLinesLimit);
      Users.deleteViaApi(testData.user.userId);
    });
  });

  it(
    'C397989 PO line numbers should be unique in duplicated orders when initial order has more than one PO line (thunderjet)',
    { tags: ['extendedPath', 'thunderjet', 'C397989', 'nonParallel'] },
    () => {
      // Step 1: Open Order from Preconditions #3 details pane
      ...
      cy.then(() => {
        ...
        const fourthPoLineNumber = `${duplicatedPoNumber}-4`;
        ...
        OrderDetails.openPolDetails(thirdPoLineNumber);
        OrderLineDetails.verifyLinesDetailTitle(`PO Line details - ${thirdPoLineNumber}`);
        ...
        OrderLines.deleteOrderLine({
          poLineNumber: thirdPoLineNumber,
