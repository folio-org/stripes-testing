...
describe('Orders', () => {
  ...
  const createOrderData = () => {
    return Organizations.createOrganizationViaApi({
      ...
      .then(() => {
        return Receiving.addPieceViaApi({
          searchParams: { createItem: 'true' },
          poLineId: testData.orderLine.id,
          poLineNumber: testData.orderLine.poLineNumber,
          format: testData.orderLine.orderFormat,
          holdingId: testData.orderLine.locations[0].holdingId,
        }).then((pieceResponse) => {
          testData.piece = pieceResponse;
        });
      })
      .then(() => createInvoiceForOrder())
      .then(() => receivePiece(testData.orderLine.id));
  };
  ...
  it(
    'C566497 Receiving a piece does not reopen one-time order with related paid invoice (Receipt status is set as "Receipt not required") (thunderjet)',
    { tags: ['extendedPath', 'thunderjet', 'C566497'] },
    () => {
      ...
      OrderLineDetails.checkFieldsConditions([
        { label: 'Receipt status', conditions: { value: 'Partially Received' } },
