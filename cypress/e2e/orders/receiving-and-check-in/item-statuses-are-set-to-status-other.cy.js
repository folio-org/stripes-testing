...
describe('orders: Receiving and Check-in', () => {
  ...
  const barcodeForFifthItem = Helper.getRandomBarcode();
  const barcodeForSixthItem = Helper.getRandomBarcode();
  const barcodeForSeventhItem = Helper.getRandomBarcode();
          ...
          .then((locationResponse) => {
            ...
            cy.createOrderApi(order)
              .then((response) => {
                ...
                ItemActions.closeItem();
                InventoryInstance.openHoldingsAccordion(location.name);
                InventoryInstance.openItemByBarcodeAndIndex('No barcode', 'row-4', 10);
                ItemActions.edit();
                ...
                ItemActions.closeItem();
                InventoryInstance.openHoldingsAccordion(location.name);
                InventoryInstance.openItemByBarcodeAndIndex('No barcode', 'row-5', 10);
                ItemActions.edit();
                ...
                ItemActions.closeItem();
                InventoryInstance.openHoldingsAccordion(location.name);
                InventoryInstance.openItemByBarcodeAndIndex('No barcode', 'row-6', 10);
                ItemActions.edit();
  ...
  it('C367971 Item statuses are set to status other than "Order closed" or "On order" and are NOT changed to "In process" upon receiving (items for receiving includes "On order" statuses) (thunderjet)', { tags: [testType.smoke, devTeams.thunderjet] }, () => {
    ...
    ItemActions.closeItem();
    InventorySearchAndFilter.switchToItem();
    InventorySearchAndFilter.searchByParameter('Barcode', barcodeForSixthItem);
    ItemRecordView.checkItemDetails(location.name, barcodeForThirdItem, ITEM_STATUS_NAMES.IN_PROCESS);
    ItemActions.closeItem();
    InventorySearchAndFilter.switchToItem();
    InventorySearchAndFilter.searchByParameter('Barcode', barcodeForSeventhItem);
