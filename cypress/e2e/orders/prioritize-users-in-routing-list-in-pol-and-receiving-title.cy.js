...
describe('Receiving', () => {
    ...
    FiscalYears.createViaApi(firstFiscalYear).then((firstFiscalYearResponse) => {
      ...
      Ledgers.createViaApi(defaultLedger).then((ledgerResponse) => {
        ...
        Funds.createViaApi(firstFund).then((fundResponse) => {
          ...
          ]).then((firstUserProperties) => {
            firstUser = firstUserProperties;
            cy.createTempUser([
              permissions.uiOrdersEdit.gui,
              permissions.uiReceivingViewEdit.gui,
                          ...
                          Organizations.createOrganizationViaApi(organization).then(
                            (responseOrganizations) => {
                              ...
                              Orders.createOrderViaApi(firstOrder).then((firstOrderResponse) => {
                                ...
                                OrderLines.fillInRoutingListInfo(routingList1);
                                OrderLines.assignUser(firstUser.username);
                                OrderLines.assignUser(secondUser.username);
                                OrderLines.assignUser(thirdUser.username);
                                OrderLines.saveRoutingList();
  ...
  it(
    'C468198 Prioritize users in Routing list in POL and Receiving title (thunderjet)',
    ...
    () => {
      ...
      OrderLines.assignUser(enterUser.username);
      OrderLines.unAssignUserFromRoutingList(firstUser.id);
      OrderLines.checkUserIsAbsent(firstUser.username);
      OrderLines.saveRoutingList();
      OrderLines.backToEditingOrder();
