...
describe('Fees&Fines', () => {
  describe('Refund Button Behavior', () => {
    const testData = {
    ...
    after('Delete test data', () => {
      ...
      // Remove service point associations before deleting users
    ...
    it(
      'C15188 Verify behavior when "Refund" button pressed from Fee/Fine Details page (vega)',
      { tags: ['extendedPath', 'vega', 'C15188'] },
      () => {
        ...
        // Verify no open or closed fees/fines are visible
        ...
        // Open Actions menu and verify Refund button is disabled
        ...
        UsersCard.openFeeFines();
        cy.wait(2000); // Wait for accordion to fully expand and buttons to become visible

        // Test all 5 fees/fines to verify Refund button is enabled for paid/transferred fees
        // Note: Partially paid/transferred are on "Open" tab, fully paid/transferred are on "Closed" tab

        // Fee/Fine 1: Partially paid - on Open tab
        UsersCard.viewAllFeesFines();
        ...
        // Fee/Fine 2: Partially transferred - on Open tab
        UserAllFeesFines.clickOnRowByIndex(1);
        FeeFineDetails.waitLoading();
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(true);
        FeeFineDetails.closeDetails();
        ...
        // Fee/Fine 5: Partially paid + partially transferred - on Open tab
        UserAllFeesFines.clickOnRowByIndex(2);
        FeeFineDetails.waitLoading();
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(true);
        FeeFineDetails.closeDetails();
        ...
        // Switch to Closed tab for fully paid/transferred fees
        UserAllFeesFines.goToClosedFeesFines();
        cy.wait(500);

        // Fee/Fine 3: Fully paid - on Closed tab
        UserAllFeesFines.clickOnRowByIndex(0);
        FeeFineDetails.waitLoading();
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(true);
        ...
        // Fee/Fine 4: Fully transferred - on Closed tab
        UserAllFeesFines.clickOnRowByIndex(1);
        FeeFineDetails.waitLoading();
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(true);
        ...
        // Perform full refund on Fee/Fine 3 (fully paid) - index 0 on Closed tab
        UserAllFeesFines.clickOnRowByIndex(0);
        FeeFineDetails.waitLoading();

        // Open Actions menu and verify Refund button is active before refund
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(true);
        ...
        // After refund, we're back on fee/fine details page
        // Verify the refund was successful and Refund button is now disabled
        FeeFineDetails.waitLoading();

        // Open Actions menu and verify Refund button is now disabled after full refund
        FeeFineDetails.openActions();
        FeeFineDetails.verifyRefundButtonState(false);
      },
    );
