  ...
  before('Create users and organizations', () => {
    ...
    cy.createTempUser([Permissions.uiOrganizationsViewEdit.gui]).then((user1) => {
      ...
      cy.createTempUser([Permissions.uiOrganizationsViewEdit.gui]).then((user2) => {
        ...
        NewOrganization.createViaApi(testData.organizations.org1).then((orgResp1) => {
          testData.organizations.org1.id = orgResp1.id;

          NewOrganization.createViaApi(testData.organizations.org2).then((orgResp2) => {
            testData.organizations.org2.id = orgResp2.id;
            ...
            OrganizationsSearchAndFilter.searchByParameters(
              'Name',
              testData.organizations.org1.name,
            );
            ...
            OrganizationsSearchAndFilter.searchByParameters(
              'Name',
              testData.organizations.org2.name,
            );
            ...
            Organizations.checkSearchResults({
              name: `${testData.organizations.org1.name}-edited`,
            });
            Organizations.selectOrganizationInCurrentPage(
              `${testData.organizations.org1.name}-edited`,
            );
            Organizations.editOrganization();
            Organizations.editOrganizationName({
              name: `${testData.organizations.org1.name}-twice`,
            });
            Organizations.saveOrganization();
            Organizations.checkOrganizationInfo({
