...
describe('Eureka', () => {
  describe('Settings', () => {
    describe('Authorization roles', () => {
      const testData = {
        roleName: `AT_C965842_UserRole_${getRandomPostfix()}`,
        capabilityColumns: [
          {
          ...
          cy.then(() => {
            cy.getApplicationsForTenantApi(Cypress.env('OKAPI_TENANT'), true).then((appIds) => {
              cy.getCapabilitySetsApi(2000, {
                ...
                capabilitySetsCountVisible = capabSets.filter((set) => set.visible).length;
              });
              cy.getCapabilitiesApi(5000, true, {
                customTimeout: 60_000,
                query: `applicationId==(${appIds.join(' or ')})`,
              }).then((capabs) => {
                capabilitiesCountVisible = capabs.filter((capab) => capab.visible).length;
              });
      ...
      it(
        'C965842 Eureka | User can add all capabs/sets to the existing role (eureka)',
        { tags: ['criticalPath', 'eureka', 'C965842'] },
        () => {
