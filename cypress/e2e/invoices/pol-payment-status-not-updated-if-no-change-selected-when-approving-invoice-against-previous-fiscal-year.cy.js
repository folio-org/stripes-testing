...
describe('Invoices', () => {
    ...
    return Orders.createOrderViaApi({
      ...
      .then((order) => {
        ...
        return OrderLines.createOrderLineViaApi(
          BasicOrderLine.getDefaultOrderLine({
            ...
            title: `AT_C700852_OrderLine_1_${getRandomPostfix()}`,
      ...
      .then((orderLine) => {
        ...
        return OrderLines.createOrderLineViaApi(
          BasicOrderLine.getDefaultOrderLine({
            purchaseOrderId: testData.order.id,
            title: `AT_C700852_OrderLine_2_${getRandomPostfix()}`,
            acquisitionMethod: testData.acquisitionMethodId,
            listUnitPrice: 30,
            poLineEstimatedPrice: 30,
            fundDistribution: [
              {
                code: testData.fund.code,
                fundId: testData.fund.id,
  ...
  it(
    'C700852 POL payment status is not updated if "No changes" option was selected when approving an invoice against a previous fiscal year (thunderjet)',
    { tags: ['extendedPath', 'thunderjet', 'C700852', 'nonParallel'] },
    () => {
      ...
      // Step 3: Pay invoice and verify Update order status modal
      InvoiceView.clickPayInActionsMenu();
      UpdatePOLinePaymentStatusModal.verifyModalView();

      // Step 4: Clcik Cancel button
      ...
      // Step 5: Select No change option
      InvoiceView.clickPayInActionsMenu();
      UpdatePOLinePaymentStatusModal.selectPaymentStatus(
        INVOICE_POL_PAYMENT_STATUSES.NO_CHANGE_UI,
      );
      ...
      // Step 10: Navigate to encumbrance details pane for the second invoice line
      OrderLines.viewPO();
      OrderDetails.waitLoading();
      OrderDetails.openInvoice(testData.invoice.id);
      InvoiceView.waitLoading();
