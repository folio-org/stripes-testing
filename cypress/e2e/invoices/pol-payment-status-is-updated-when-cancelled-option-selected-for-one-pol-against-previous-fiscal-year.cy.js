...
describe('Invoices', () => {
  ...
  const getOrderLine = (orderLineKey, price) => {
    return BasicOrderLine.getDefaultOrderLine({
      ...
      title: `AT_C709266_${orderLineKey}_${getRandomPostfix()}`,
  ...
  const createOrderWithLines = () => {
    return Orders.createOrderViaApi({
      ...
      .then((order) => {
        ...
        return OrderLines.createOrderLineViaApi(getOrderLine('orderLine1', 20));
      })
      .then((orderLine) => {
        testData.orderLine1 = orderLine;

        return OrderLines.createOrderLineViaApi(getOrderLine('orderLine2', 30));
      })
      .then((orderLine) => {
  ...
  it(
    'C709266 POL payment status is updated when "Cancelled" option is selected for one PO line when cancelling an invoice against a previous fiscal year (thunderjet)',
    { tags: ['extendedPath', 'thunderjet', 'C709266', 'nonParallel'] },
    () => {
      ...
      // Step 2: Navigate to POL #2 details pane
      ...
      // Step 4: Approve and pay invoice and verify Update order status modal
      InvoiceView.clickApproveAndPayInvoice({ isApprovePayEnabled: true });
      UpdatePOLinePaymentStatusModal.verifyModalView();

      // Step 5: Select Cancelled option
      UpdatePOLinePaymentStatusModal.selectPaymentStatus(INVOICE_POL_PAYMENT_STATUSES.CANCELLED);
      ...
      // Step 9: Check encumbrance for the current FY
