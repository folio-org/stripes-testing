...
const testData = {
  user: {},
  sharedMarcInstance: {
    title: `AT_C422189_SharedMarcInstance_${getRandomPostfix()}`,
  },
  localMarcInstance: {
    title: `AT_C422189_LocalMarcInstance_${getRandomPostfix()}`,
  },
  college: {
    locationId: null,
    sourceId: null,
  ...
  describe('Consortia', () => {
    describe('GetRecord', () => {
      ...
      before('Create test data', function () {
        ...
        cy.createMarcBibliographicViaAPI(
          ...
          InventoryHoldings.createHoldingRecordViaApi({
            ...
            testData.college.holdingsId = holdingsData.id;
          });
          ...
          InventoryHoldings.createHoldingRecordViaApi({
            ...
            testData.university.holdingsId = holdingsData.id;
          });
        });
        ...
        // Create user and login to Central tenant
      ...
      after('Delete test data', () => {
        ...
        // Delete College holdings and local MARC instance
        cy.setTenant(Affiliations.College);
        if (testData.college.holdingsId) {
          cy.deleteHoldingRecordViaApi(testData.college.holdingsId);
        }
        ...
        cy.setTenant(Affiliations.University);
        if (testData.university.holdingsId) {
          cy.deleteHoldingRecordViaApi(testData.university.holdingsId);
        }
        Behavior.updateBehaviorConfigViaApi();
      ...
      it(
        'C422189 Consortia | SRS+Inventory | GetRecord | Marked as Deleted shared MARC Instances from Central tenant is retrieved in the response of single tenant harvest (consortia) (firebird)',
        { tags: ['extendedPathECS', 'firebird', 'C422189', 'nonParallel'] },
        () => {
          // Step 1-3: Mark shared MARC instance as deleted from Central tenant
          ...
          // Step 4: Verify shared MARC instance with marc21 format - deleted status
          cy.resetTenant();
          cy.getAdminToken();
          cy.setTenant(Affiliations.College);
          ...
          // Step 5: Verify shared MARC instance with marc21_withholdings format - deleted status
          OaiPmhEdge.getRecordRequest(
            testData.sharedMarcInstance.uuid,
            Affiliations.College,
            'marc21_withholdings',
