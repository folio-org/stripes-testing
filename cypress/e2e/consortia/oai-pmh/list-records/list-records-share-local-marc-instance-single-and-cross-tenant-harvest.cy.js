...
const testData = {
  user: {},
  marcInstance: {
    title: `AT_C402375_LocalMarcInstance_${getRandomPostfix()}`,
    uuid: null,
    hrid: null,
  },
  holdingsId: null,
  ...
  describe('ListRecords', () => {
    describe('Consortia', () => {
      ...
      after('Delete test data', () => {
        cy.resetTenant();
        cy.getAdminToken();
        // Delete holdings and shared instance from College tenant
        cy.setTenant(Affiliations.College);
      ...
      it(
        ...
        () => {
          ...
          // Step 3-4: Verify instance NOT in current date OAI-PMH response before sharing
          cy.resetTenant();
          cy.getAdminToken();
          cy.setTenant(Affiliations.College);
          ...
          InventoryInstance.verifyCalloutMessage(
            `Local instance ${testData.marcInstance.title} has been successfully shared`,
          );
          InventoryInstance.checkSharedTextInDetailView();
          InventoryInstance.getAssignedHRID().then((updatedHrid) => {
            expect(testData.marcInstance.hrid).not.equal(updatedHrid);
            testData.marcInstance.hrid = updatedHrid;
          });

          // Step 6: Verify single-tenant ListRecords marc21 for College tenant
          cy.resetTenant();
          cy.getAdminToken();
          cy.setTenant(Affiliations.College);
          ...
          // Step 10: Verify cross-tenant ListRecords marc21_withholdings for Central tenant
          OaiPmhEdge.listRecordsRequest(
            'marc21_withholdings',
            OaiPmhEdge.getApiKey(Affiliations.Consortia),
          ).then((response) => {
          ...
          // Step 11: Verify cross-tenant ListRecords oai_dc for Central tenant
          OaiPmhEdge.listRecordsRequest(
            'oai_dc',
            OaiPmhEdge.getApiKey(Affiliations.Consortia),
          ).then((response) => {
