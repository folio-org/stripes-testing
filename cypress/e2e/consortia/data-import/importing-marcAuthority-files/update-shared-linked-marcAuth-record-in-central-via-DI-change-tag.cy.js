  ...
  describe('Importing MARC Authority files', () => {
    describe('Consortia', () => {
      ...
      it(
        ...
        () => {
          // Step 1-2: Verify authority record exists and export it
          ...
          // Step 3-4: Export authority record for modification
          ...
          // Step 5: Modify exported MARC file to change tag 100 to 110
          cy.readFile(`cypress/fixtures/${testData.exportedMarcFileName}`, null).then((buffer) => {
            // Convert buffer to string for manipulation
            const marcString = buffer.toString('binary');
            // Replace tag 100 with 110 in the MARC record
            // MARC format: tag is 3 ASCII characters in the directory section
            const modifiedString = marcString.replace(/100(\d{4}\d{5})/g, '110$1');
            // Write back as buffer
            cy.writeFile(
              `cypress/fixtures/${testData.exportedMarcFileName}`,
              Buffer.from(modifiedString, 'binary'),
              { encoding: null },
            );
          });

          // Step 6-7: Import modified MARC file with updated tag via Data Import
