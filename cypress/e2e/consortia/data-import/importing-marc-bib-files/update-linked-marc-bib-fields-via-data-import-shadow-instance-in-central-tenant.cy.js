...
describe('Data Import', () => {
  describe('Importing MARC Bib files', () => {
    describe('Consortia', () => {
      ...
      const authData = { prefix: randomLetters, startWithNumber: 411803 };
      ...
      const bibTitle = `AT_C411803_MarcBibInstance_${randomPostfix}`;
      const authorityHeadingPrefix = `AT_C411803_MarcAuthority_${randomPostfix}`;

      const bibSubfields650 = {
        a: 'C411803 Good',
      };
      const bibSubfields711 = {
        a: 'C411803 Mostly Festival',
        e: 'Orch',
        4: 'prf',
        v: 'version 1',
      };
      ...
      const field711AttemptedAValue = 'C411803 Mostly Updated';
      ...
      const mappingProfile = { name: `AT_C411803_Update MARC Bib ${randomPostfix}` };
      const actionProfile = {
        name: `AT_C411803_Update MARC Bib ${randomPostfix}`,
        action: 'UPDATE',
        folioRecordType: EXISTING_RECORD_NAMES.MARC_BIBLIOGRAPHIC,
      };
      const matchProfile = {
        profileName: `AT_C411803_Match 999 ff $s ${randomPostfix}`,
        incomingRecordFields: { field: '999', in1: 'f', in2: 'f', subfield: 's' },
        existingRecordFields: { field: '999', in1: 'f', in2: 'f', subfield: 's' },
        recordType: EXISTING_RECORD_NAMES.MARC_BIBLIOGRAPHIC,
      };
      const jobProfile = {
        profileName: `AT_C411803_Update MARC Bib by matching 999 ff $s ${randomPostfix}`,
      };

      const csvFile = `AT_C411803_export${randomPostfix}.csv`;
      const exportedMarcFile = `AT_C411803_exportedMarcFile${randomPostfix}.mrc`;
      const modifiedMarcFile = `AT_C411803_modifiedMarcFile${randomPostfix}.mrc`;
      const importedFileName = `AT_C411803_importedFile${randomPostfix}.mrc`;
      ...
      before('Create test data', () => {
        ...
        InventoryInstances.deleteFullInstancesByTitleViaApi('C411803_');
        cy.resetTenant();
        MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C411803_');
        InventoryInstances.deleteFullInstancesByTitleViaApi('C411803_');
        ...
        cy.then(() => {
          ...
          .then(() => {
            // Create job profile in Central tenant
            cy.resetTenant();
          ...
          .then(() => {
            // Export the shared instance from Central tenant
          ...
          .then(() => {
            // User: primary affiliation = Central; also Member 1 (College) and Member 2 (University)
      ...
      it(
        'C411803 Updating subfields in a linked field of shared "MARC Bib" which has "Shadow" copy in member tenant via Data Import from Central tenant (promin)',
        { tags: ['extendedPathECS', 'promin', 'C411803'] },
        () => {
          // Steps 1, 9-10: Already in Central; search and verify updated linked fields via View source
          ...
          // 650: controlled $a, $0, then surviving $2 additions, with $9 repositioned last by Data Import
          ...
          // 711: controlled $a/$e/$t, $0, then updated $4, with $9 repositioned last by Data Import
          InventoryViewSource.checkRowExistsWithTagAndValue(
            tags.tag711,
          ...
          // Steps 11-13: Switch to Member 2 tenant; verify same fields in Edit MARC bibliographic record
          InventoryInstance.waitLoading();
          InventoryInstance.waitInstanceRecordViewOpened();
