...
describe('Data Import', () => {
  describe('Importing MARC Bib files', () => {
    describe('Consortia', () => {
      ...
      const authData = { prefix: randomLetters, startWithNumber: 407693 };
      ...
      const bibTitle = `AT_C407693_MarcBibInstance_${randomPostfix}`;
      const authorityHeadingPrefix = `AT_C407693_MarcAuthority_${randomPostfix}`;

      const bibSubfields650 = {
        a: 'C407693 Good',
      };
      const bibSubfields711 = {
        a: 'C407693 Mostly Festival',
        e: 'Orch',
        4: 'prf',
        v: 'version 1',
      };
      ...
      const field711AttemptedAValue = 'C407693 Mostly Updated';
      ...
      const mappingProfile = { name: `AT_C407693_Update MARC Bib ${randomPostfix}` };
      const actionProfile = {
        name: `AT_C407693_Update MARC Bib ${randomPostfix}`,
        action: 'UPDATE',
        folioRecordType: EXISTING_RECORD_NAMES.MARC_BIBLIOGRAPHIC,
      };
      const matchProfile = {
        profileName: `AT_C407693_Match 999 ff $s ${randomPostfix}`,
        incomingRecordFields: { field: '999', in1: 'f', in2: 'f', subfield: 's' },
        existingRecordFields: { field: '999', in1: 'f', in2: 'f', subfield: 's' },
        recordType: EXISTING_RECORD_NAMES.MARC_BIBLIOGRAPHIC,
      };
      const jobProfile = {
        profileName: `AT_C407693_Update MARC Bib by matching 999 ff $s ${randomPostfix}`,
      };

      const csvFile = `AT_C407693_export${randomPostfix}.csv`;
      const exportedMarcFile = `AT_C407693_exportedMarcFile${randomPostfix}.mrc`;
      const modifiedMarcFile = `AT_C407693_modifiedMarcFile${randomPostfix}.mrc`;
      const importedFileName = `AT_C407693_importedFile${randomPostfix}.mrc`;
      ...
      before('Create test data', () => {
        cy.resetTenant();
        cy.getAdminToken();
        MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C407693_');
        InventoryInstances.deleteInstanceByTitleViaApi('C407693_');
      ...
      it(
        'C407693 Updating subfields in a linked field of shared "MARC Bib" in member tenant via Data Import (promin)',
        { tags: ['extendedPathECS', 'promin', 'C407693'] },
        () => {
