...
const testData = {
  user: {},
  listName: `AT_C506691_List_${getRandomPostfix()}`,
  sharedInstances: [],
  localInstance: { id: null, title: `AT_C506691_LocalInstance_${getRandomPostfix()}` },
  instanceTypeId: null,
  ...
  describe('Consortia', () => {
    describe('Instances', () => {
      before('Create test data', () => {
        ...
        // Create a local instance in College tenant to ensure results on clean env
        cy.then(() => {
        ...
        // Create shared instances in central tenant
        cy.then(() => {
          for (let i = 0; i < 2; i++) {
            const title = `AT_C506691_SharedInstance${i + 1}_${getRandomPostfix()}`;
            InventoryInstances.createFolioInstanceViaApi({
      ...
      it(
        'C506691 Verify that when Instance sharing is local then ONLY instances that are NOT shadow instances are returned (consortia) (athena)',
        { tags: ['extendedPathECS', 'athena', 'C506691'] },
        () => {
          ...
          // Step 6: Select "Local" value
          QueryModal.chooseValueSelect('Local');
          QueryModal.verifySelectedValue('Local');
          QueryModal.verifyQueryAreaContent('(instance.shared == Local)');
          ...
          QueryModal.verifyQueryAreaContent(
            `(instance.shared == Local) AND (instance.created_at == ${todayDate})`,
          );
          ...
          // Step 9: Verify "Instance — Shared" column shows "Local" for all rows
          QueryModal.verifyColumnDisplayed(instanceFieldValues.instanceShared);
          Lists.verifyQueryValue('Local', QUERY_OPERATIONS.EQUAL, 'list-column-instance.shared');
        },
      );
    });
  });
