...
const testData = {
  name: `AT_C436904_AuthSourceFile_${randomPostfix}`,
  prefix: randomLetters,
  startsWith: '1000',
  baseUrl: `https://autotest/C436904/${randomPostfix}/`,
  isActive: true,
  date: DateTools.getFormattedDateWithSlashes({ date: new Date() }),
  updatedName: `AT_C436904_AuthSourceFile_UPD_${randomPostfix}`,
  updatedPrefix: `${randomLetters}U`,
  updatedStartsWith: '11111',
  updatedBaseUrl: `https://autotest-upd/C436904/${randomPostfix}/`,
  updatedIsActive: false,
};
const marcAuthorityTabName = 'MARC authority';
const manageAuthFilesOption = 'Manage authority files';
const authorityFields = [
  { tag: '100', content: `$a AT_C436904_MarcAuthority_${randomPostfix}`, indicators: ['\\', '\\'] },
];
      ...
      describe('Consortia', () => {
        before(
          ...
          () => {
            cy.resetTenant();
            cy.getAdminToken();
            MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('AT_C436904_');
            // Try to delete previously created source files, or at least deactivate them
            cy.getAuthoritySourceFileDataViaAPI('AT_C436904_*').then(() => {
        ...
        it(
          'C436904 Edit all fields of Local "Authority file" which has assigned "MARC authority" records at Central tenant only, from Central tenant (consortia) (spitfire)',
          { tags: ['criticalPathECS', 'spitfire', 'nonParallel', 'C436904'] },
          () => {
            ...
            // Step 6: Switch to Member tenant, verify updated values
