  ...
  describe('Consortium manager', () => {
    describe('Manage local settings', () => {
      describe('Manage local Contributor types', () => {
        ...
        let userB;
        const testData = {
          contributorTypeName: `AT_C411524_ContributorType_${getRandomPostfix()}`,
          contributorTypeCode: `AT_C411524_${getRandomPostfix()}`,
          editedContributorTypeCode: `AT_C411524_${getRandomPostfix()}_edited`,
        };
        ...
        before('Create test data', () => {
          ...
          // Create User A in College tenant (member-1) per TestRail preconditions
          cy.setTenant(Affiliations.College);
          cy.createTempUser([Permissions.uiOrganizationsView.gui]).then((userProperties) => {
            userA = userProperties;

            cy.resetTenant();
            // Assign User A affiliations to University (member-2)
            ...
            // Assign User A permissions in University tenant (member-2)
            ...
            cy.createTempUser([Permissions.crudContributorTypes.gui]).then((userBProperties) => {
              ...
              // User A logs in and switches to Central tenant
              cy.resetTenant();
        ...
        after('Delete test data', () => {
          cy.resetTenant();
          cy.getAdminToken();
          // Delete User A from College tenant (where created)
          cy.setTenant(Affiliations.College);
          Users.deleteViaApi(userA.userId);
          // Delete User B from Central tenant (where created)
          cy.resetTenant();
        ...
        it(
          'C411524 User with "Consortium manager: Can share settings to all members" permission can manage local contributor types via "Consortium manager" app (consortia) (thunderjet)',
          { tags: ['extendedPathECS', 'thunderjet', 'C411524'] },
          () => {
            ...
            // Verify Share checkbox is enabled (different from restrictions test)
            ...
            // Step 14: Edit central tenant contributor type
            ...
            // Step 15: Clear code field using x button
            ConsortiaControlledVocabularyPaneset.clearTextField('code');

            // Step 16: Try to save with empty code field to verify validation
            ConsortiaControlledVocabularyPaneset.clickSave();
            ConsortiaControlledVocabularyPaneset.verifyFieldValidatorError({
            ...
            // Step 18: Save the edited contributor type
            ConsortiaControlledVocabularyPaneset.clickSave();
            ConsortiumManagerApp.checkMessage(
              messages.updated(testData.contributorTypeName, tenantNames.central),
            );
            ...
            // Step 19: Delete contributor type from University tenant
            ConsortiaControlledVocabularyPaneset.performActionFor(
              testData.contributorTypeName,
              tenantNames.university,
              actionIcons.trash,
            ...
            // Step 22: Verify contributor type in Central tenant Settings
            ...
            // Step 24: Switch to University tenant and verify deleted contributor type NOT present
