...
const authorityUUIDsFileName = `AT_C558415_authority_uuids_${getRandomPostfix()}.csv`;
const title = `AT_C558415_MarcAuthority_${getRandomPostfix()}`;
const fields = [
  {
    tag: '100',
    content: `$a ${title}`,
...
const authFile = {
  sourceName: `AT_C558415_AuthoritySourceFile_${getRandomPostfix()}`,
          ...
          const assertionsOnMarcFileContent = [
            {
              ...
              assertions: [
                ...
                (record) => expect(record.get('999')[0].subf[0][1]).to.eq(sharedMarcAuthIds[0]),
              ],
            },
            {
              uuid: deletedLocalMarcAuthId,
              assertions: [
                (record) => expect(record.get('100')[0].subf[0][0]).to.eq('a'),
                (record) => expect(record.get('100')[0].subf[0][1]).to.eq(title),
                (record) => expect(record.get('999')[0].subf[1][0]).to.eq('s'),
                (record) => expect(record.get('999')[0].subf[0][0]).to.eq('i'),
                (record) => expect(record.get('999')[0].subf[0][1]).to.eq(localMarcAuthIds[0]),
              ],
            },
          ];
