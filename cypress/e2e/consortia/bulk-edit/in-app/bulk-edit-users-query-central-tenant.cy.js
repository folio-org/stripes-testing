...
describe('Bulk-edit', () => {
  describe('Query approach - Consortia', () => {
    before('create test data', () => {
      ...
      cy.createTempUser([
        ...
        // Preconditions: Build query to find staff users
    ...
    it(
      ...
      () => {
        ...
        cy.wait('@getPreview', getLongDelay()).then((interception) => {
          ...
          // Expected: Preview CSV file saved with correct content
          BulkEditActions.downloadPreview();
          const expectedEmail = user.username.replace(/@.*/, `@${newEmailDomain}`);
          ExportFile.verifyFileIncludes(previewQueryFileName, [user.username, expectedEmail]);

          // Step 14: Click "Commit changes" button
          // Expected: Records updated successfully with banner notification
          BulkEditActions.commitChanges();
          BulkEditActions.verifySuccessBanner(1);
          BulkEditSearchPane.verifyChangedResults(expectedEmail);

          // Step 15: Click "Actions" menu => Select "Download changed records (CSV)" element
          // Expected: Changed records CSV file saved
          BulkEditActions.openActions();
          BulkEditActions.downloadChangedCSV();
          ExportFile.verifyFileIncludes(changedRecordsQueryFileName, [
            user.username,
            expectedEmail,
          ]);

          // Step 16: Navigate to "Users" app => Search for edited Users => Verify changes
          // Expected: Changes applied to Users
          TopMenuNavigation.navigateToApp(APPLICATION_NAMES.USERS);
          UsersSearchPane.searchByUsername(user.username);
          UsersSearchPane.openUser(user.username);
          UsersCard.openContactInfo();
          UsersCard.verifyEmail(expectedEmail);
