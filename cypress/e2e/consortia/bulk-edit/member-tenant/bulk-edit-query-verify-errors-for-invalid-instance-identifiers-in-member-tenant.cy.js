...
const instances = [...Array(2)].map(() => ({
  title: `AT_C965839_FolioInstance_${getRandomPostfix()}`,
}));
    ...
    describe('Consortia', () => {
      ...
      it(
        ...
        () => {
          ...
          // Step 2: Select Instance UUID field, "in" operator, enter Instance UUIDs
          ...
          // Step 4: Click "Run query" and check the result
          ...
          cy.wait('@errors', getLongDelay()).then(() => {
            ...
            // Step 7: Click "Download errors (CSV)"
          ...
          // Step 8: Return to Bulk edit, open Query tab, build new query with Instance HRID
          ...
          // Step 10: Click "Run query" and check the result
          cy.intercept('GET', '**/errors?limit=10&offset=0&errorType=ERROR').as('errorsHrid');
          QueryModal.clickRunQuery();
          QueryModal.verifyClosed();

          cy.wait('@errorsHrid', getLongDelay()).then(() => {
            ...
            // Step 12: Click "Actions" menu => Download errors (CSV)
            BulkEditActions.openActions();
            BulkEditActions.downloadErrors();
