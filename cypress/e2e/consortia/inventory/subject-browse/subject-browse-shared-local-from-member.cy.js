...
describe('Inventory', () => {
  describe('Subject Browse', () => {
    describe('Consortia', () => {
      ...
      const testData = {
        instanceTitlePrefix: `AT_C410758_Instance_${randomPostfix}`,
        searchOption: searchInstancesOptions[1],
        subjectPrefix: `AT_C410758_Subject_${randomPostfix}`,
      ...
      const localM2Instances = instances.filter((inst) => inst.title.includes('LocalM2'));
      let user;

      before('Create test data', () => {
        cy.resetTenant();
        cy.getAdminToken();
        InventoryInstances.deleteInstanceByTitleViaApi('AT_C410758');
      ...
      it(
        ...
        () => {
          ...
          [...sharedInstances, ...localM1Instances].forEach((instance) => {
            BrowseSubjects.checkRowWithValueAndNoAuthorityIconExists(instance.subject);
          });
          localM2Instances.forEach((instance) => {
            BrowseSubjects.checkResultIsAbsent(instance.subject);
          });

          BrowseSubjects.browse(localM2Instances[0].subject);
          BrowseSubjects.verifyNonExistentSearchResult(localM2Instances[0].subject);
          ...
          BrowseSubjects.checkSearchResultRecord(localM1Instances[0].subject);

          BrowseSubjects.openInstance({ name: localM1Instances[0].subject });
          ...
          BrowseSubjects.openInstance({ name: localM1Instances[1].subject });
          InventorySearchAndFilter.verifySearchResult(localM1Instances[1].title);
          InventorySearchAndFilter.checkRowsCount(1);
          ConsortiumManager.checkCurrentTenantInTopMenu(tenantNames.college);
        },
      );
