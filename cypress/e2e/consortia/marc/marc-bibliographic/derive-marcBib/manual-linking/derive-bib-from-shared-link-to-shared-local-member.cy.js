    ...
    describe('Derive MARC bib', () => {
      describe('Manual linking', () => {
        describe('Consortia', () => {
          ...
          const authorityHeadingShared2 = `AT_C410748_MarcAuthority_${randomPostfix}_Shared2`;
          const authorityHeadingLocal = `AT_C410748_MarcAuthority_${randomPostfix}_Local`;
          ...
          const bibFields = [
            ...
            {
              tag: testData.tag100,
              content: `$a AT_C410748_MarcAuthority_${randomPostfix} Field100 $e author`,
              indicators: ['1', '\\'],
            },
            {
              tag: testData.tag600,
              content: `$a AT_C410748_MarcAuthority_${randomPostfix} Field600 $d 1739-1792 $x Assassination`,
              indicators: ['1', '0'],
            },
            {
              tag: testData.tag700,
              content: `$a AT_C410748_MarcAuthority_${randomPostfix} Field700`,
              indicators: ['\\', '\\'],
            },
          ];
          ...
          before('Create test data', () => {
            cy.then(() => {
              cy.resetTenant();
              cy.getAdminToken();
              MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('AT_C410748');
              InventoryInstances.deleteInstanceByTitleViaApi('AT_C410748');
              ...
              .then(() => {
                cy.setTenant(Affiliations.College);
                cy.createTempUser(userPremissions)
                  .then((createdUserProperties) => {
                    user = createdUserProperties;

                    cy.resetTenant();
                    cy.assignPermissionsToExistingUser(user.userId, userPremissions);
                  })
                  .then(() => {
                    cy.setTenant(Affiliations.College);
