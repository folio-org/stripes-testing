    ...
    describe('Edit', () => {
      describe('Consortia', () => {
        ...
        const testData = {
          ...
          tag980: '980',
          tag999: '999',
          authorityNaturalId: `n552476${randomNDigitNumber(17)}`,
          authorityHeadingPrefix: `AT_C552476_MarcAuthority_${randomPostfix}`,
          requiredField980Content: '$a Required first field 980',
          requiredField980Content2: '$a Required second field 980',
          errorMessage: 'Fail: Field is non-repeatable.',
          field008Index: 3,
        };

        const auth010Content = `$a ${testData.authorityNaturalId}`;
        ...
        it(
          'C552476 Created Local MARC authority record on Member tenant is verified against Member tenant rules (consortia) (spitfire)',
          { tags: ['criticalPathECS', 'spitfire', 'C552476', 'nonParallel'] },
          () => {
            cy.then(() => {
              cy.resetTenant();
              cy.getAdminToken();
              MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('AT_C552476_');
                  ...
                  cy.getSpecificationFields(memberSpecId).then((response) => {
                    ...
                    if (existingField980) {
                      ...
                      cy.createSpecificationField(
                        ...
                        {
                          tag: testData.tag980,
                          label: `AT_C552476_Custom_Field_${randomPostfix}`,
                          url: 'http://www.example.org/C552476field980.html',
                          repeatable: false,
                          required: true,
                          deprecated: false,
                        },
              ...
              .then(() => {
                ...
                QuickMarcEditor.addNewField(
                  testData.tag010,
                  auth010Content,
                  testData.field008Index,
                );
                QuickMarcEditor.checkContentByTag(testData.tag010, auth010Content);

                QuickMarcEditor.addNewField(
                  testData.tag100,
                  auth100Content,
                  testData.field008Index + 1,
                );
                QuickMarcEditor.checkContentByTag(testData.tag100, auth100Content);

                QuickMarcEditor.addNewField(
                  testData.tag980,
                  testData.requiredField980Content,
                  testData.field008Index + 2,
                );
                QuickMarcEditor.checkContent(
                  testData.requiredField980Content,
                  testData.field008Index + 3,
                );
                QuickMarcEditor.verifyTagValue(testData.field008Index + 3, testData.tag980);
                ...
                QuickMarcEditor.checkContent(
                  testData.requiredField980Content2,
                  testData.field008Index + 4,
                );
                QuickMarcEditor.verifyTagValue(testData.field008Index + 4, testData.tag980);
                ...
                // Step 7: Add required field 500
                QuickMarcEditor.deleteField(testData.field008Index + 4);
                QuickMarcEditor.verifyTagValue(testData.field008Index + 4, testData.tag999);

                // Step 8: Click "Save & close" — should succeed
                QuickMarcEditor.pressSaveAndCloseButton();
                QuickMarcEditor.checkAfterSaveAndCloseAuthority();
                MarcAuthority.contains(auth100Content);
