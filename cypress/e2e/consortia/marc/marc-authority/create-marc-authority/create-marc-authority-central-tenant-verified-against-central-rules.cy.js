    ...
    describe('Edit', () => {
      describe('Consortia', () => {
        ...
        const testData = {
          ...
          tag500: '500',
          tag980: '980',
          authorityNaturalId: `n552475${randomNDigitNumber(17)}`,
          authorityHeadingPrefix: `AT_C552475_MarcAuthority_${randomPostfix}`,
        ...
        it(
          'C552475 Created MARC authority record on Central tenant is verified against Central tenant rules (consortia) (spitfire)',
          { tags: ['criticalPathECS', 'spitfire', 'C552475', 'nonParallel'] },
          () => {
            cy.then(() => {
              cy.resetTenant();
              cy.getAdminToken();
              MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('AT_C552475_');
                  ...
                  cy.getSpecificationFields(memberSpecId).then((response) => {
                    ...
                    if (existingField980) {
                      ...
                      cy.createSpecificationField(
                        ...
                        {
                          tag: testData.tag980,
                          label: `AT_C552475_Custom_Field_${randomPostfix}`,
                          url: 'http://www.example.org/C552475field980.html',
                          repeatable: false,
                          required: true,
                          deprecated: false,
                        },
              ...
              .then(() => {
                ...
                QuickMarcEditor.addNewField(testData.tag010, auth010Content, 3);
                QuickMarcEditor.checkContentByTag(testData.tag010, auth010Content);

                QuickMarcEditor.addNewField(testData.tag100, auth100Content, 4);
