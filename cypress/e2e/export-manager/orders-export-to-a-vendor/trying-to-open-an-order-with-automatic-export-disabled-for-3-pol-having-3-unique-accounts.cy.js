...
describe('Export Manager', () => {
  describe('Export Orders in EDIFACT format: Orders Export to a Vendor', () => {
    ...
    after(() => {
      cy.getAdminToken();
      Orders.deleteOrderViaApi(order.id).then(() => {
        if (location) {
    ...
    it(
      'C350437 Check that a User is not alerted trying to open an Order with automatic-export disabled for 3 POLs, having 3 unique accounts (thunderjet) (TaaS)',
      { tags: ['extendedPath', 'thunderjet', 'C350437'] },
      () => {
