...
describe('MARC Bibliographic Validation Rules - Cannot Create Subfield with Invalid Required Value API', () => {
  ...
  const LOCAL_FIELD_TAG = '968';
  const TEST_CASE_ID = 'C499719';
    ...
    cy.createTempUser(requiredPermissions).then((createdUser) => {
      ...
      getBibliographicSpec().then((bibSpec) => {
        ...
        const testData = fieldTestDataBuilder
          .withField(LOCAL_FIELD_TAG, 'Local Field for Invalid Required Test', {
            url: 'http://www.example.org/field968.html',
            repeatable: true,
            required: false,
            deprecated: false,
          })
  ...
  it(
    'C499719 Cannot create Subfield code of Local field with invalid "required" field value for MARC bib spec (API) (spitfire)',
    { tags: ['C499719', 'extendedPath', 'spitfire'] },
    () => {
      ...
      cy.createSpecificationFieldSubfield(localFieldId, subfieldPayload, false).then(
        (createResponse) => {
          expect(createResponse.status, 'Should return 400 for invalid required value').to.eq(400);
