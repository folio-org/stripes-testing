...
describe('MARC Bibliographic Validation Rules - Cannot Create Local Field with Invalid Repeatable Value API', () => {
  ...
  it(
    'C490932 Cannot create Local Field with invalid value in "repeatable" field for MARC bib spec (API) (spitfire)',
    { tags: ['C490932', 'extendedPath', 'spitfire'] },
    () => {
      // Ensure token is set for the user before API calls
      cy.getUserToken(user.username, user.password);

      // Test scenarios for repeatable field validation
      const repeatableValidationScenarios = [
        {
          description: 'Repeatable field with unrecognized token "test" (should fail)',
          tag: '899',
          repeatable: 'test',
          expectedStatus: 400,
      ...
      // Test each repeatable field validation scenario
      repeatableValidationScenarios.forEach((scenario) => {
        const payload = {
          ...baseFieldPayload,
          tag: scenario.tag,
