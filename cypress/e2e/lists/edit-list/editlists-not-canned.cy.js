...
describe('"Edit lists" functionality for not canned lists', () => {
  const userData = {};
  const listData = {
  ...
  it('C411734 Edit list: Make the list Inactive (corsair)', { tags: ['smoke', 'corsair'] }, () => {
    cy.login(userData.username, userData.password);
    cy.visit(TopMenu.listsPath);
    Lists.waitLoading();
    Lists.openNewListPane();
    ...
    cy.contains('Warning: making status inactive will clear list contents.').should('be.visible');
    Lists.saveList();
