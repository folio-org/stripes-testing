  ...
  describe('Query Builder', () => {
        ...
        Orders.createOrderViaApi(order1).then((ord1) => {
          ...
          Orders.updateOrderViaApi({ ...ord1, workflowStatus: ORDER_STATUSES.OPEN }).then(() => {
            ...
            Invoices.createInvoiceViaApi({
              ...
              // Pay Invoice #1
              Invoices.changeInvoiceStatusViaApi({ invoice: inv1, status: INVOICE_STATUSES.PAID });
        ...
        const createOrderWithPol = (vendor, { listUnitPrice = 10 } = {}) => {
          ...
          return Orders.createOrderViaApi(order).then((ord) => {
            ...
            return OrderLines.createOrderLineViaApi(pol).then((polResp) => {
              return Orders.updateOrderViaApi({ ...ord, workflowStatus: ORDER_STATUSES.OPEN }).then(
                () => ({ order: ord, pol: polResp }),
              );
            });
          });
        };
    ...
    it(
      ...
      () => {
        ...
        QueryModal.verifyRecordWithIdentifierAbsentInResultTable(testData.invoice2.folioInvoiceNo);
        QueryModal.verifyRecordWithIdentifierAbsentInResultTable(testData.invoice3.folioInvoiceNo);
        QueryModal.verifyRecordWithIdentifierAbsentInResultTable(testData.invoice4.folioInvoiceNo);
        QueryModal.verifyRecordWithIdentifierAbsentInResultTable(testData.invoice5.folioInvoiceNo);
