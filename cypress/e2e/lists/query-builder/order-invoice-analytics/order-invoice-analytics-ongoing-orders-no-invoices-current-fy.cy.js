  ...
  describe('Query Builder', () => {
    before('Create test data', () => {
      ...
      cy.then(() => {
        ...
        createOngoingOrderWithPol({ listUnitPrice: 10 }).then(({ order: ord1, pol: pol1Resp }) => {
        ...
        createOngoingOrderWithPol({ listUnitPrice: 10 }).then(({ order: ord2, pol: pol2Resp }) => {
          testData.order2 = ord2;
          testData.pol2 = pol2Resp;
        });

        // ── Order #3 (Ongoing, 1 POL, re-encumber=disabled) ──────────────────
        createOngoingOrderWithPol({ listUnitPrice: 10 }).then(({ order: ord3, pol: pol3Resp }) => {
    ...
    after('Delete test data', () => {
      ...
      [testData.order1, testData.order2, testData.order3, testData.order4, testData.order5].forEach(
        (ord) => {
