...
describe('Lists', () => {
  describe('Query Builder UI', () => {
    ...
    const verifyQueryBuilder = (field, operator, filedType, value, query) => {
      ...
      Lists.verifyQueryValue(value, operator);
    ...
    describe('Organizations', () => {
      ...
      it('C451507 Search for "organizations" in the query builder using "Status" field (corsair)',
        { tags: ['criticalPath', 'corsair', 'C451507'] },
        () => {
          listName = getTestEntityValue('C451507_List');
          openQueryBuilder(recordType);
          verifyQueryBuilder('Organization — Status', QUERY_OPERATIONS.EQUAL, 'select', 'Active', 'organization.status == Active');
        });
    });
    ...
    describe('Purchase order lines', () => {
      ...
      it('C451553 Verify that grouped fields display within a list row for "POL — Fund distribution" column (corsair)',
        { tags: ['criticalPath', 'corsair', 'C451553'] },
        () => {
          listName = getTestEntityValue('C451553_List');
          openQueryBuilder(recordType);
          verifyQueryBuilder('POL — UUID', QUERY_OPERATIONS.NOT_IN, 'input', 'test', 'pol.id not in test');
        });
    });
  });
});
