...
describe('Lists', () => {
  describe('Query Builder', () => {
    describe('Custom fields', () => {
      ...
      describe('Textbox custom field queryable, updatable, and deletable', () => {
        const customFieldValue = 'AT_C648489_TextboxValue';
        let userData = {};
        let listName;
        const testData = {
        ...
        it(
          ...
          () => {
            ...
            // #3 Click "Select field" dropdown and search for the added textbox custom field
        ...
        it(
          ...
          () => {
            ...
            // Update the textbox custom field name via API
            cy.getAdminToken();
        ...
        it(
          ...
          () => {
            ...
            // Create a new list with a query using the textbox custom field (use current label after update)
            const currentLabel = testData.updatedFieldLabel || testData.customFieldLabel;
            ...
            QueryModal.selectField(currentLabel);
            QueryModal.verifySelectedField(currentLabel);
            QueryModal.selectOperator(QUERY_OPERATIONS.EQUAL);
            QueryModal.fillInValueTextfield(customFieldValue);
            QueryModal.testQuery();
            QueryModal.waitForQueryTestToFinish();
            ...
            // Delete the textbox custom field via API
            cy.getAdminToken();
            cy.deleteCustomFieldsViaApi({ ids: [testData.customField.id] });
            testData.customField = null;

            // Wait for the deletion to propagate to the entity type
            waitForFieldLabelToBeAbsent(currentLabel);

            // Go to the "Lists" app and open the list that contained the textbox custom field
            cy.visit(TopMenu.listsPath);
            Lists.waitLoading();
            Lists.openList(listName);
            ...
            Lists.getQueryText().then((queryText) => {
              expect(queryText).not.to.include(currentLabel);
            });
