...
const testCaseId = 'C1453666';
const listName = `AT_${testCaseId}_List_${getRandomPostfix()}`;
const countryMatch = 'USA';
const countryNoMatch = 'CAN';
const countrySearchValue = 'US';
const orgDescription = `AT_${testCaseId}_Description_${getRandomPostfix()}`;

const testData = {
  orgWithCountryMatch: {},
  orgWithCountryNoMatch: {},
};
let user;
    ...
    describe('Organizations', () => {
      before('Create test data and login', () => {
        ...
        Organizations.createOrganizationViaApi({
          ...
          addresses: [{ country: countryMatch }],
        }).then((id) => {
          testData.orgWithCountryMatch.id = id;
          Organizations.getOrganizationByIdViaApi(id).then((org) => {
            testData.orgWithCountryMatch.code = org.code;
          });
        });
        ...
        Organizations.createOrganizationViaApi({
          ...
          description: orgDescription,
          addresses: [{ country: countryNoMatch }],
        }).then((id) => {
          testData.orgWithCountryNoMatch.id = id;
          Organizations.getOrganizationByIdViaApi(id).then((org) => {
            testData.orgWithCountryNoMatch.code = org.code;
          });
        });
      ...
      after('Delete test data', () => {
        ...
        cy.getAdminToken();
        if (testData.orgWithCountryMatch?.id) {
          Organizations.deleteOrganizationViaApi(testData.orgWithCountryMatch.id);
        }
        if (testData.orgWithCountryNoMatch?.id) {
          Organizations.deleteOrganizationViaApi(testData.orgWithCountryNoMatch.id);
        }
        if (user?.userId) Users.deleteViaApi(user.userId);
      });

      it(
        "C1453666 Search for 'Organizations' using Addresses - Country - operator: starts with (athena)",
        { tags: ['extendedPath', 'athena', 'C1453666'] },
        () => {
          ...
          // Step 3: Select "Organizations - Addresses - Country" field
          QueryModal.selectField(ORGANIZATIONS_FIELDS.ORGANIZATION.ADDRESSES.COUNTRY);

          // Step 4: Verify operators list for Country field
          ...
          // Step 6: Fill in value "US"
          QueryModal.fillInValueTextfield(countrySearchValue);
          ...
          // Step 8: Verify preview - org with country USA IS shown, org with country CAN is NOT shown
          ...
          QueryModal.verifyOrganizationAddressesEmbeddedTableInQueryModal(
            testData.orgWithCountryMatch.code,
            {
              country: countryMatch,
            },
          );
          QueryModal.verifyRecordWithIdentifierAbsentInResultTable(
            testData.orgWithCountryNoMatch.code,
          );
          ...
          Lists.verifyRecordValueAbsentInResultTable(testData.orgWithCountryNoMatch.code);
        },
      );
    });
  });
