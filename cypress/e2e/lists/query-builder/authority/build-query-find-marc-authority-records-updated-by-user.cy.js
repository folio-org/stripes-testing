  ...
  describe('Query Builder', () => {
    describe('Authority', () => {
      ...
      const testData = {
        ...
        listName: `AT_C1322597_List_${randomPostfix}`,
        heading1: `AT_C1322597_MarcAuthority_1_${randomPostfix}`,
        heading2: 'AT_C1322597_MarcAuthority_2_Imported',
        email: `at_c1322597_${getRandomLetters(7)}@test.com`,
      };

      const marcFile = {
        marc: 'marcAuthFileC1322597.mrc',
        fileName: `testMarcAuthC1322597.${getRandomPostfix()}.mrc`,
        jobProfileToRun: DEFAULT_JOB_PROFILE_NAMES.CREATE_AUTHORITY,
      };

      const authData = {
        prefix: getRandomLetters(10),
        startWithNumber: `1322597${randomNDigitNumber(6)}`,
      };
      ...
      before('Create test data', () => {
        cy.getAdminToken();
        MarcAuthorities.deleteMarcAuthorityByTitleViaAPI('C1322597_');
        ...
        cy.createTempUser([], undefined, undefined, undefined, testData.email)
          ...
          .then(() => {
            // Create records under admin token — createdByUserId = admin
          ...
          .then(() => {
            // Switch to user token — edits here will set updatedByUserId = test user
          ...
          .then(() => {
            cy.getMarcRecordDataViaAPI(authorityId1).then((marcRecord) => {
              marcRecord.fields.push({
                tag: '500',
                content: '$a AT_C1322597 User note 1',
                indicators: ['\\', '\\'],
              });
          ...
          .then(() => {
            cy.getMarcRecordDataViaAPI(authorityId2).then((marcRecord) => {
              marcRecord.fields.push({
                tag: '500',
                content: '$a AT_C1322597 User note 2',
                indicators: ['\\', '\\'],
              });
              cy.updateMarcRecordDataViaAPI(marcRecord.parsedRecordId, marcRecord);
            });
      ...
      it(
        'C1322597 Build query to find MARC authority records updated by a user (spitfire)',
        { tags: ['extendedPath', 'spitfire', 'C1322597'] },
        () => {
          ...
          // Step 2: Updated by user — Username equals test user username → both records found
          ...
          // Step 5: Created by user — Username equals test user username → no records found
          // (records were created by admin, not the test user)
          ...
          // Step 3: Updated by user — Last name, first name equals test user → both records found
          ...
          // Step 6: Created by user — Last name, first name equals test user → no records found
          ...
          // Step 4: Updated by user — Email equals test user email → both records found
          ...
          // Step 7: Created by user — Email equals test user email → no records found
