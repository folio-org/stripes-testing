...
describe('Check out', () => {
  const testData = {
    ...
    note: {
      title: `AT_C380718_Note_${getRandomPostfix()}`,
      details: `This is Note 1 - ${getRandomPostfix()}`,
      checkoutApp: true,
      usersApp: false,
    },
    folioInstances: InventoryInstances.generateFolioInstances({
      instanceTitlePrefix: `AT_C380718_FolioInstance_${getRandomPostfix()}`,
      itemsCount: 2,
    }),
  };
  ...
  it(
    'C380718 Verify that note of second user will pop-up when first check outed user didn\'t have notes (using "End session" button) (spitfire)',
    { tags: ['extendedPath', 'spitfire', 'C380718'] },
    () => {
      ...
      UsersCard.openNotesSection();
      UsersCard.verifyNoteInAccordion(testData.note);
      ...
      // Step 10: Click on the "End session" button
      CheckOutActions.endCheckOutSession();
      // Verify "Scan patron card" and "Scan items" panes are displayed without values
      CheckOutActions.checkIsInterfacesOpened();

      // Step 11: Fill in barcode value of user that has assigned "Note" records (User B)
      // Step 12: Click on the "Enter" button
      ...
      // Step 13: Click on the "Close" button
      CheckOutActions.closeNote();
      // Verify the modal window with the note is closed
      CheckOutActions.checkNoteModalNotDisplayed();
      CheckOutActions.checkPatronInformation();

      // Step 14: Fill in the "Item" barcode value in input field at "Scan items" pane → Click "Enter" button
