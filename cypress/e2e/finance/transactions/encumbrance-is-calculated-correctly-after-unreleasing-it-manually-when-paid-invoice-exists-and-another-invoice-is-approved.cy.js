...
describe('Finance', () => {
  describe('Transactions', () => {
    ...
    const createInvoice = ({ invoiceKey, subTotal, invoiceStatus }) => {
      return OrderLines.getOrderLineByIdViaApi(testData.orderLine.id)
        ...
        .then((invoice) => {
          ...
          if (invoiceStatus !== INVOICE_STATUSES.OPEN) {
            Invoices.changeInvoiceStatusViaApi({ invoice, status: invoiceStatus });
          }
        });
    };
    ...
    it(
      'C1045391 Encumbrance is calculated correctly after unreleasing it manually when a paid invoice exists and another invoice is approved (release encumbrance = true) (thunderjet)',
      { tags: ['extendedPath', 'thunderjet', 'C1045391', 'nonParallel'] },
      () => {
        ...
        TransactionDetails.checkTransactionDetails({
          information: [
            ...
            { key: TRANSACTION_DETAIL_FIELDS.FROM, value: testData.fund.name },
            { key: TRANSACTION_DETAIL_FIELDS.INITIAL_ENCUMBRANCE, value: '$10.00' },
            { key: TRANSACTION_DETAIL_FIELDS.AWAITING_PAYMENT, value: '$6.00' },
            { key: TRANSACTION_DETAIL_FIELDS.EXPENDED, value: '$3.00' },
            { key: TRANSACTION_DETAIL_FIELDS.STATUS, value: ENCUMBRANCE_STATUSES.RELEASED },
          ],
        });

        // Step 5: Unrelease the encumbrance
        Funds.unreleaseEncumbrance();
        TransactionDetails.checkTransactionDetails({
          information: [
            { key: TRANSACTION_DETAIL_FIELDS.AMOUNT, value: '($1.00)' },
            { key: TRANSACTION_DETAIL_FIELDS.INITIAL_ENCUMBRANCE, value: '$10.00' },
            { key: TRANSACTION_DETAIL_FIELDS.AWAITING_PAYMENT, value: '$6.00' },
            { key: TRANSACTION_DETAIL_FIELDS.EXPENDED, value: '$3.00' },
            { key: TRANSACTION_DETAIL_FIELDS.STATUS, value: ENCUMBRANCE_STATUSES.UNRELEASED },
        ...
        // Step 7: Open the encumbrance from the invoice line of Invoice #2
        InvoiceView.selectInvoiceLine();
        InvoiceLineDetails.checkFundDistibutionTableContent([
        ...
        // Step 9: Open the encumbrance from the invoice line of Invoice #1
        InvoiceView.selectInvoiceLine();
        InvoiceLineDetails.checkFundDistibutionTableContent([
