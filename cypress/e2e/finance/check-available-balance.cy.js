...
describe('Finance', () => {
  ...
  before(() => {
    ...
    Orders.createOrderForRollover(defaultOrder).then(firstOrderResponse => {
      ...
      OrderLines.rolloverPOLineInfoforPhysicalMaterialWithFund(defaultFund, '1100', '1', '1100', location.institutionId);
    ...
    cy.createTempUser([
      ...
      .then(userProperties => {
        user = userProperties;
        cy.login(userProperties.username, userProperties.password, { path:TopMenu.fiscalYearPath, waiter: FiscalYears.waitForFiscalYearDetailsLoading });
      });
  });
  ...
  it('C377030 "Available balance" is displayed as a negative number when running a deficit (thunderjet)', { tags: [testType.criticalPath, devTeams.thunderjet] }, () => {
