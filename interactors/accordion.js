...
export default HTML.extend('accordion')
  ...
  .filters({
    ...
    itemsAmount: (el) =>
      el.querySelector('[class^=mclEndOfListContainer-]').getAttribute('data-end-of-list'),
    contentId: (el) => {
    ...
    textareaNames: (el) =>
      [...el.querySelectorAll('textarea')].map((textarea) => textarea.getAttribute('name')),
    selectNames: (el) =>
      [...el.querySelectorAll('select')].map((select) => select.getAttribute('name')),
    ...
    buttonIds: (el) =>
      [...el.querySelectorAll('div[class^=content-] button')]
        .map((button) => button.getAttribute('id'))
        .filter((id) => id),
    buttonContainsText: (el) =>
      [...el.querySelectorAll('div[class^=content-] button')]
        .map((button) => button.textContent)
        .filter((id) => id),
    inputNames: (el) =>
      [...el.querySelectorAll('input')].map((input) => input.getAttribute('name')),
