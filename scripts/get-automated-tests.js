...
function printAutomatedTestsByTeamsFromTestRun() {
  getTests().then((tests) => {
    ...
    for (const key in team) {
      const count = automatedTests.filter((test) => test.custom_dev_team === team[key]).length;
...
function printAutomatedTestsByTeams() {
  getTestCases().then((cases) => {
    ...
    const actualTestCases = cases.filter(
      (caseItem) => caseItem.custom_release !== null &&
    ...
    console.log(
      `Number of actual tests cases (smoke, critical, extended, backend, edge cases) in the project #${projectId} run: ${actualTestCases.length}`,
    );
    // custom_automation_type: 1 - Automated, 2 - Manual
    const automatedTests = actualTestCases.filter(
      (caseItem) => caseItem.custom_automation_type === 1,
    );
    const manualTests = actualTestCases.filter((caseItem) => caseItem.custom_automation_type === 2);
    console.log(
      `Number of automated tests cases in the project #${projectId} run: ${automatedTests.length}`,
    );
    console.log(
      `Number of manual tests cases in the project #${projectId} run: ${manualTests.length}`,
    );
    ...
    for (const key in team) {
      ...
      console.log(
        `${key.length <= 4 ? key + '  ' : key}: \t${automatedCount}\t\t${manualCount}\t${coverage}%`,
      );
      fileJson.teams.push({
        team: key,
    ...
    console.log(
      `Total: \t\t${totalAutomated}\t\t${totalManual}\t${((totalAutomated / (totalAutomated + totalManual)) * 100).toFixed(2)}%`,
    );
    fs.writeFile(
      `${fileJson.date.replace(/:/g, '_').replace(/\./g, '_').replace(/-/g, '_')}_automated_tests.json`,
